#!/bin/bash
# c182 FIX CYCLE - R1282 zrfF v2. The c181 cycle: delivery and gates all
# worked (patch fetched byte-exact, markers exact, backup made) and run.sh
# correctly refused a stale binary - but the patch itself was broken: the
# door was anchored on the LAST TWO LINES of the three-line status return
# expression, splicing the block mid-expression (clang: expected ';'
# after return statement). Zero R1282 evidence; the tree is now the
# broken 7282b44e with the good backup beside it.
# v2 places the door BEFORE the complete return statement. NEW VALIDATION:
# the sandbox PARSED the patched file before shipping (gcc -fsyntax-only,
# stubbed mach-o, zero errors, identical error set vs baseline) - the
# Mac compile is now the SECOND parser to read it, not the first.
# This script: restore from backup (drift-gated), fetch+apply v2 with
# post-apply SHA gate, 120s run with alive-guards (captures skipped if the
# run dies early - no stale-artifact captures), censuses, PNG last.
set -u
cd "$HOME/Downloads/xenolift" || exit 1

echo "===PRE182=== identity, restore the baseline tree (drift-gated)"
T=$(shasum -a 256 runtime/runtime.c | cut -d" " -f1)
echo "TREE_SHA=$T"
BASE="05f05543cdd68fe1648129a24acc92d225d022c1477891cbd1a074c7845935c5"
BROKEN="7282b44ec92c609f8f43b91ab42580ba93aefab16e165a28f3854ee1141886d8"
if [ "$T" = "$BROKEN" ]; then
  B=$(shasum -a 256 runtime/runtime.c.bak1282 | cut -d" " -f1)
  echo "BACKUP_SHA=$B"
  if [ "$B" != "$BASE" ]; then
    echo "RESTORE-FAILED: backup is not the receipted baseline - nothing run"
    exit 0
  fi
  cp -p runtime/runtime.c.bak1282 runtime/runtime.c
  echo "RESTORED from runtime/runtime.c.bak1282"
elif [ "$T" != "$BASE" ]; then
  echo "DRIFT-FAILED: tree is neither the c181 broken state nor the baseline - nothing run"
  exit 0
fi
T=$(shasum -a 256 runtime/runtime.c | cut -d" " -f1)
if [ "$T" != "$BASE" ]; then
...
echo "===PATCH182=== fetch, verify, apply the R1282 v2 patch - marker + post-apply sha gates, FAIL EXPLICIT"
curl -sSL -m 30 -o /tmp/r1282_zrfF_v2.diff "https://base44.app/api/apps/6aa26c7ce0ac9a5ee05d2b95/files/mp/public/6aa26c7ce0ac9a5ee05d2b95/a0de03671_r1282_zrfF_v2.diff"
echo "CURL_RC=$?"
P=$(shasum -a 256 /tmp/r1282_zrfF_v2.diff | cut -d" " -f1)
echo "PATCH_SHA=$P"
if [ "$P" != "71e62bfe9a92b88bfb9ee95071b08d14913f2ea931ee01c20b00211ae05b5df6" ]; then
  echo "PATCH-VALID-FAILED: sha mismatch - nothing applied, nothing run"
  exit 0
fi
M1=$(grep -c "zrfF" /tmp/r1282_zrfF_v2.diff)
M2=$(grep -c "R1282" /tmp/r1282_zrfF_v2.diff)
...
if ! patch -p0 --dry-run runtime/runtime.c < /tmp/r1282_zrfF_v2.diff; then
  echo "DRYRUN-FAILED: patch does not apply cleanly - nothing changed"
  exit 0
fi
if ! patch -p0 runtime/runtime.c < /tmp/r1282_zrfF_v2.diff; then
  cp -p runtime/runtime.c.bak1282 runtime/runtime.c
...
echo "NEW_SHA=$N"
if [ "$N" != "6e16afe76fcce8b718600a885c42af65d55e08e3c7e121a4aa76743786c32d41" ]; then
  cp -p runtime/runtime.c.bak1282 runtime/runtime.c
  echo "POST-APPLY-FAILED: patched tree sha mismatch (expected 6e16afe7) - restored backup, nothing run"
  exit 0
fi
echo "PATCHED TREE VERIFIED (sandbox-parsed, sha-gated)"

echo "===RUN182=== the 120s verification run - alive-guarded captures, live-tail correlation"
RUN_BUDGET_S=120 ./run.sh > /tmp/run182.txt 2>&1 &
RUNPID=$!
echo "LAUNCH-WALL $(date +%s)"
sleep 6
if ! kill -0 $RUNPID 2>/dev/null; then
  echo "RUN-DIED-EARLY: compile or launch failed - no captures, no stale artifacts"
  head -8 /tmp/run182.txt
  wait $RUNPID
  echo "RUNSH_RC=$?"
  echo "===END182==="
  exit 0
...
LV_B="vram_live.bin"
LV_M="vram_live.meta"
SNAP=$(mktemp -d /tmp/xl182.XXXXXX)
...
echo "RUNSH_RC=$?"
head -3 run.log
tail -12 /tmp/run182.txt

echo "===SHOTS182=== render with error retention, verify, preserve"
...
echo "===SCHED182=== the scheduled-flag observations"
grep -n "cdw02" run.log | tail -6
grep -c "pendclr" run.log
echo "===WALL182=== the wall census"
grep -c "NULL-trap" run.log
grep -c "AbortOnGameFault" run.log
echo "===PROV182=== provenance"
LC_ALL=C grep -a -o "runtime build R[0-9]*" run.log | head -1
wc -l run.log
cp -p run.log run.log.c182
shasum -a 256 run.log run.log.c182 runtime/runtime.c
echo "===PNG182=== THE IMAGE TRANSFER - small renders, sha-verified, budget-bounded, LAST so the census always lands"
python3 - "$SNAP" <<'PYEOF'
