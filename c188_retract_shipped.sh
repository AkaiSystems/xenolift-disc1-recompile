#!/bin/bash
# c188 THE R502 RETRACTION - R1288, the first-fault fix, ONE targeted change.
...
if [ "$M1" != "3" ] || [ "$M2" != "2" ] || [ "$M3" != "2" ] || [ "$M4" -lt "2" ]; then
  echo "MARKER-FAILED: expected R1288=3 rlgl=2 latchfix=2 RRG>=2 - nothing applied"
  exit 0
fi
