#!/bin/bash
# c192 THE MOVIE-QUEUE DELIVERY CAMERA - R1291, cameras only, NO behavior.
# THE c191 VERDICT: the run went DEEPER THAN EVER - no boot restarts until
# the very end, member 18 (the MOVIE MODULE) installed with gdisp=6 (dest
# 801EF300), asciiart fired, the screen held real structured content
# across all four witnesses (308K nonzero pixels), 2.19 BILLION vram
# writes, the sync band served (239324), file#18 stalled load COMPLETED
# by the defib6 carry - and the game wedged ALIVE at THE MOVIE RE-REQUEST
# POSTURE: FE04=0x1AA66 (109158=file#18, stamped by the game own ftab
# reader), FDF8=0, act=0, pend=0, stale sched=1, FE1C=10, A22C=5, drive
# parked at seek=3, the game polling GetStat in the movie-loader family.
# THE TWO DECLINING DOORS, both named: zrfB matches every term except the
# command byte (band 09, this poll is 01); the R496 never-armed arm
# matches file-table members against cd_seek_lba (the parked directory
# position) instead of FE04 (the queued request). THE OPEN QUESTION the
# c193 fix hangs on: does the movie dest buffer ALREADY HOLD the shelved
# file#18 data (the wait is the movie PLAY protocol) or is it EMPTY (a
# delivery fault)? R1291 prints the answer AT the wedge: stuck-confirmed
# posture -> FE04/FE08 request pair, file-table member for FE04, shelved
# state, first 8 words + 4KB nonzero count at the request dst. PASS =
# receipts only: dest-held -> decode the play-protocol wait next;
# dest-empty -> the porter/delivery fix (or the R496-arm FE04 widen).
...
echo "===PRE192=== identity, tree drift gate (R1290 baseline)"
T=$(shasum -a 256 runtime/runtime.c | cut -d" " -f1)
echo "TREE_SHA=$T"
BASE="6e57555af37e17bd87820613cd5aa14bc410b0237f89945958eb29e5494f5235"
if [ "$T" != "$BASE" ]; then
  echo "DRIFT-FAILED: tree is not the receipted R1290 baseline 6e57555a - nothing run"
  exit 0
fi
T=$(shasum -a 256 runtime/runtime.c | cut -d" " -f1)
if [ "$T" != "$BASE" ]; then
...
echo "===PATCH192=== fetch, verify, apply the R1291 movie-queue camera patch - marker + post-apply sha gates, FAIL EXPLICIT"
curl -sSL -m 30 -o /tmp/r1291_movq.diff "https://base44.app/api/apps/6aa26c7ce0ac9a5ee05d2b95/files/mp/public/6aa26c7ce0ac9a5ee05d2b95/9c839d523_r1291_movq.diff"
echo "CURL_RC=$?"
P=$(shasum -a 256 /tmp/r1291_movq.diff | cut -d" " -f1)
echo "PATCH_SHA=$P"
if [ "$P" != "9966e997d3b8a43ff1a0f3959ddeb161aff5ceb77aff99572154b70b7740edbe" ]; then
  echo "PATCH-VALID-FAILED: sha mismatch - nothing applied, nothing run"
  exit 0
fi
M1=$(grep -c "R1291" /tmp/r1291_movq.diff)
M2=$(grep -c "movq" /tmp/r1291_movq.diff)
M3=$(grep -c "movie-queue" /tmp/r1291_movq.diff)
M4=$(grep -c "dest-empty" /tmp/r1291_movq.diff)
echo "PATCH_MARKERS R1291=$M1 movq=$M2 movie-queue=$M3 dest-empty=$M4"
if [ "$M1" != "3" ] || [ "$M2" != "1" ] || [ "$M3" != "1" ] || [ "$M4" != "3" ]; then
  echo "MARKER-FAILED: expected R1291=3 movq=1 movie-queue=1 dest-empty=3 - nothing applied"
  exit 0
fi
...
echo "NEW_SHA=$N"
if [ "$N" != "57c45fdaa475c7d75576b91a197febe0af42743c24c02584a7569921e435425c" ]; then
  cp -p runtime/runtime.c.bak1291 runtime/runtime.c
  echo "POST-APPLY-FAILED: patched tree sha mismatch (expected 57c45fda) - restored backup, nothing run"
  exit 0
fi
echo "PATCHED TREE VERIFIED (sandbox-parsed, sha-gated)"

echo "===RUN192=== the 120s receipt run - alive-guarded captures, live-tail correlation"
RUN_BUDGET_S=120 ./run.sh > /tmp/run192.txt 2>&1 &
RUNPID=$!
echo "LAUNCH-WALL $(date +%s)"
sleep 6
if ! kill -0 $RUNPID 2>/dev/null; then
  echo "RUN-DIED-EARLY: compile or launch failed - no captures, no stale artifacts"
  head -8 /tmp/run192.txt
  wait $RUNPID
  echo "RUNSH_RC=$?"
  echo "===END192==="
  exit 0
...
LV_B="vram_live.bin"
LV_M="vram_live.meta"
SNAP=$(mktemp -d /tmp/xl192.XXXXXX)
...
echo "RUNSH_RC=$?"
head -3 run.log
tail -12 /tmp/run192.txt

echo "===SHOTS192=== render with error retention, verify, preserve"
...
echo "===STORM192=== THE GOVERNOR VERDICT - the storm must stay tamed (6.6MB class last run)"
...
echo "===MOVQ192=== THE MOVIE-QUEUE DELIVERY VERDICT - the dest-held vs dest-empty answer"
grep -c "movq" run.log
grep -n "movq" run.log | head -16
...
echo "===ZRFG2192=== THE FILE-BAND SERVE - fires if the file#14 posture recurs"
...
echo "===ALARM192=== the alarm-deferral + FE1C/FE04 writer trails"
grep -c "alarmguard" run.log
...
echo "===WALL192=== the wall census"
grep -c "NULL-trap" run.log
grep -c "AbortOnGameFault" run.log
echo "===PROV192=== provenance"
LC_ALL=C grep -a -o "runtime build R[0-9]*" run.log | head -1
wc -l run.log
cp -p run.log run.log.c192
shasum -a 256 run.log run.log.c192 runtime/runtime.c
echo "===PNG192=== THE IMAGE TRANSFER - small renders, sha-verified, budget-bounded, LAST so the census always lands"
python3 - "$SNAP" <<'PYEOF'
