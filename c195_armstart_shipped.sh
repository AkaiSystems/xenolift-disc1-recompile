#!/bin/bash
# c195 THE NEVER-STARTED ARMED READ - R1298 ARMSTART, ONE targeted
# behavior change (the gameplay pivot: produce game play). THE c196 RUN
# RECEIPTS (tree d6e3d013, all gates green, 166s cycle): (1) THE c194 ARM
# CYCLE WORKED THE QUEUE DEEPER THAN EVER: the game own stepper drove the
# field band (FE1C states 0-5-6-A-1-2-0 by fn 800413EC - R285 movie/file
# stepper EXECUTED), fetch entries #391-400 act=1 DRAINING (FDF8 63864 ->
# 55672), LBAs 108939-108971 CONSUMED (fldsec receipts), member 14 installed
# (dest 801DD680, the field module, gdisp=1) - then the terminal wedge moved
# TO A NEW CLASS, receipted 3x by the R1283 decline camera at 30s cadence:
# FDF8=92180 ARMED at FE04=108995 (file#14 territory) BUT the drive parked at
# seek=108933, act=0, FIFO empty, pend=0, sched=0, cmd=02 - THE ARMED READ
# NEVER STARTED. defib6 correctly declined (its rescue requires act=1;
# stuffing a ring the drive never took = the c174 lesson). THE MISSING
# TRANSITION IS THE START: stamp seek=FE04, act=1, same-poll first-sector
# serve (cd_data_load), pend=1, force INT1 - the proven zrfB composite,
# DRIVE-STATE ONLY: FE1C never written, sched never cleared, FDF8 never
# touched (the arm is the game own). GUARD RAILS: one fire per request LBA
# (8-slot ring), band LBAs only (100000-300000), sane armed sizes
# (2048-125304), 30s frozen confirm, budget 4. REVERT per the c174 lesson:
# fire with zero guest-side consumption (FDF8 never drains below the armed
# size, no fldsec at the new LBA, no FE1C movement) = revert next cycle.
# (2) THE JOS BRANCH QUESTION ANSWERED by the fixed R1104: the dispatch
# target is VALID (gdesc[0] real cells: fn=8001A4B4 the menu, mem=800592B8,
# heapstart=8006FAEC) - per the Jos rule, attention shifts back to the wait
# and response sequence, which is exactly where this wedge sits. (3) The
# reshtail camera printed (293B) - the 8-byte delta content receipts land in
# the log for the next read. NOTE: the queued diagnostic-integrity directive
# has STALE base gates (its bases are 57c45fda/928f393d; the tree is now
# d6e3d013 and moves to 1082fd90 with this patch) - it will DRIFT-FAIL
# cleanly if it runs; the integrity cameras rebase onto the armstart tree
# next cycle. Base gate d6e3d013 (the c196 provenance tree), patch sha
# 436da680 (sandbox-verified to apply clean and parse, 0 -Wformat warnings),
# post-apply 1082fd90, backup and restores read .bak1298. Bootstrap: fetch,
# SHA-256 gate, syntax gate, provenance copy, execute.

cd "$HOME/Downloads/xenolift" || exit 1

echo "===PRE195=== identity, tree drift gate (c194-arm baseline d6e3d013 - the tree the c196 cycle receipted by provenance)"
T=$(shasum -a 256 runtime/runtime.c | cut -d" " -f1)
echo "TREE_SHA=$T"
BASE="d6e3d01384d8057f0bc8053aeb6ac0caea71d1881e8a4e8a6e68a78e5860222b"
if [ "$T" != "$BASE" ]; then
  echo "DRIFT-FAILED: tree is not the receipted c194-arm baseline d6e3d013 (the c196 run tree, provenance-verified) - nothing run"
  exit 0
fi
T=$(shasum -a 256 runtime/runtime.c | cut -d" " -f1)
if [ "$T" != "$BASE" ]; then
...
echo "===PATCH195=== fetch, verify, apply the R1298 armstart patch - marker + post-apply sha gates, FAIL EXPLICIT"
curl -sSL -m 30 -o /tmp/r1298_armstart.diff "https://base44.app/api/apps/6aa26c7ce0ac9a5ee05d2b95/files/mp/public/6aa26c7ce0ac9a5ee05d2b95/76f281dd7_r1298_armstart.diff"
echo "CURL_RC=$?"
P=$(shasum -a 256 /tmp/r1298_armstart.diff | cut -d" " -f1)
echo "PATCH_SHA=$P"
if [ "$P" != "436da6800b9c1ffaf56587e64e0b96bff7efe43e8c2fd31a47097ee2c0aab626" ]; then
  echo "PATCH-VALID-FAILED: sha mismatch - nothing applied, nothing run"
  exit 0
fi
M1=$(grep -c "R1298" /tmp/r1298_armstart.diff)
M2=$(grep -c "armstart" /tmp/r1298_armstart.diff)
echo "PATCH_MARKERS R1298=$M1 armstart=$M2"
if [ "$M1" != "2" ] || [ "$M2" != "2" ]; then
  echo "MARKER-FAILED: expected R1298=2 armstart=2 - nothing applied"
  exit 0
fi
cp -p runtime/runtime.c runtime/runtime.c.bak1298
if ! patch -p0 --dry-run runtime/runtime.c < /tmp/r1298_armstart.diff; then
  echo "DRYRUN-FAILED: patch does not apply cleanly - nothing changed"
  exit 0
fi
if ! patch -p0 runtime/runtime.c < /tmp/r1298_armstart.diff; then
  cp -p runtime/runtime.c.bak1298 runtime/runtime.c
  echo "APPLY-FAILED: restored backup, nothing run"
  exit 0
fi
N=$(shasum -a 256 runtime/runtime.c | cut -d" " -f1)
echo "NEW_SHA=$N"
if [ "$N" != "1082fd904a86ea9cf1a394791c00c0e0bc2daaa71fe1e18f1b0f5f8bcbe82ec1" ]; then
  cp -p runtime/runtime.c.bak1298 runtime/runtime.c
  echo "POST-APPLY-FAILED: patched tree sha mismatch (expected 1082fd90) - restored backup, nothing run"
  exit 0
fi
echo "PATCHED TREE VERIFIED (sandbox-parsed, sha-gated)"

echo "===RUN195=== the 120s receipt run - alive-guarded captures, live-tail correlation"
...
LV_B="vram_live.bin"
LV_M="vram_live.meta"
SNAP=$(mktemp -d /tmp/xl195.XXXXXX)
...
echo "===SHOTS195=== render with error retention, verify, preserve"
...
echo "===STORM195=== THE GOVERNOR VERDICT - the storm must stay tamed (6.6MB class last run)"
...
echo "===ARMSTART195=== THE R1298 VERDICT - the never-started armed read STARTED (or the decline receipts stand)"
...
echo "===PROV195=== provenance"
LC_ALL=C grep -a -o "runtime build R[0-9]*" run.log | head -1
wc -l run.log
cp -p run.log run.log.c195
shasum -a 256 run.log run.log.c195 runtime/runtime.c
echo "===PNG195=== THE IMAGE TRANSFER - small renders, sha-verified, budget-bounded, LAST so the census always lands"
python3 - "$SNAP" <<'PYEOF'
