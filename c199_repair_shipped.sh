...
# warnings), post-apply e80fc808, backup and restores read .bak1301.
# Bootstrap: fetch, SHA-256 gate, syntax gate, provenance copy,
# execute.

cd "$HOME/Downloads/xenolift" || exit 1

echo "===PRE197=== identity, tree drift gate (c196-mvw baseline e00520d4 - the tree the c198 cycle receipts by provenance)"
T=$(shasum -a 256 runtime/runtime.c | cut -d" " -f1)
echo "TREE_SHA=$T"
BASE="e00520d43f883268cd0d0221ebc80d0785a964f881e51a958d0072fb9482307c"
if [ "$T" != "$BASE" ]; then
  echo "DRIFT-FAILED: tree is not the receipted c196-mvw baseline e00520d4 (the c198 run tree, provenance-verified) - nothing run"
  exit 0
fi
T=$(shasum -a 256 runtime/runtime.c | cut -d" " -f1)
if [ "$T" != "$BASE" ]; then
...
echo "===PATCH199=== fetch, verify, apply the R1299b+R1301 combined patch - marker + post-apply sha gates, FAIL EXPLICIT"
curl -sSL -m 30 -o /tmp/r1299b_r1301.diff "https://base44.app/api/apps/6aa26c7ce0ac9a5ee05d2b95/files/mp/public/6aa26c7ce0ac9a5ee05d2b95/a52976bae_r1299b_r1301.diff"
echo "CURL_RC=$?"
P=$(shasum -a 256 /tmp/r1299b_r1301.diff | cut -d" " -f1)
echo "PATCH_SHA=$P"
if [ "$P" != "e98e7e1b1645f948c12335ad5845e1cd363cddd753966729ae2356ee117f45ff" ]; then
  echo "PATCH-VALID-FAILED: sha mismatch - nothing applied, nothing run"
  exit 0
fi
M1=$(grep -c "R1299" /tmp/r1299b_r1301.diff)
M2=$(grep -c "freezedoss" /tmp/r1299b_r1301.diff)
M3=$(grep -c "ARM-CONTEXT" /tmp/r1299b_r1301.diff)
M4=$(grep -c "actchg" /tmp/r1299b_r1301.diff)
M5=$(grep -c "R1301" /tmp/r1299b_r1301.diff)
M6=$(grep -c "repair-the-branch" /tmp/r1299b_r1301.diff)
echo "PATCH_MARKERS R1299=$M1 freezedoss=$M2 fdw=$M3 actchg=$M4 R1301=$M5 repair=$M6"
if [ "$M1" != "7" ] || [ "$M2" != "1" ] || [ "$M3" != "1" ] || [ "$M4" != "1" ] || [ "$M5" != "1" ] || [ "$M6" != "1" ]; then
  echo "MARKER-FAILED: expected R1299=7 freezedoss=1 fdw=1 actchg=1 R1301=1 repair=1 - nothing applied"
  exit 0
fi
cp -p runtime/runtime.c runtime/runtime.c.bak1301
if ! patch -p0 --dry-run runtime/runtime.c < /tmp/r1299b_r1301.diff; then
  echo "DRYRUN-FAILED: patch does not apply cleanly - nothing changed"
  exit 0
fi
if ! patch -p0 runtime/runtime.c < /tmp/r1299b_r1301.diff; then
  cp -p runtime/runtime.c.bak1301 runtime/runtime.c
  echo "APPLY-FAILED: restored backup, nothing run"
  exit 0
fi
N=$(shasum -a 256 runtime/runtime.c | cut -d" " -f1)
echo "NEW_SHA=$N"
if [ "$N" != "e80fc808cbddb44e69ec6372bea221d6ccc408d2535fdc70137515ffd9d3397b" ]; then
  cp -p runtime/runtime.c.bak1301 runtime/runtime.c
  echo "POST-APPLY-FAILED: patched tree sha mismatch (expected e80fc808) - restored backup, nothing run"
  exit 0
fi
echo "PATCHED TREE VERIFIED (sandbox-parsed, sha-gated)"

echo "===RUN199=== the 120s receipt run - alive-guarded captures, live-tail correlation"
...
LV_B="vram_live.bin"
LV_M="vram_live.meta"
SNAP=$(mktemp -d /tmp/xl199.XXXXXX)
...
echo "===SHOTS199=== render with error retention, verify, preserve"
...
echo "===STORM199=== THE GOVERNOR VERDICT - the storm must stay tamed (6.6MB class last run)"
...
echo "===MVWAIT197=== THE R1298 VERDICT (stays 0-fire unless its class recurs) + THE R1300 MOVIE-WAIT DELIVERY VERDICT"
...
echo "===FDW199=== THE FDF8 ARM WRITER TRAIL (the 92180 arm names itself: writer fn + requested LBA + dest + drive posture at arm)"
grep -c "fdw" run.log
...
echo "===PROV199=== provenance"
LC_ALL=C grep -a -o "runtime build R[0-9]*" run.log | head -1
wc -l run.log
cp -p run.log run.log.c199
shasum -a 256 run.log run.log.c199 runtime/runtime.c
echo "===PNG199=== THE IMAGE TRANSFER - small renders, sha-verified, budget-bounded, LAST so the census always lands"
python3 - "$SNAP" <<'PYEOF'
