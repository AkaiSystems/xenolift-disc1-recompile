#!/bin/bash
# c199 THE GATE-REPAIR BUILD - R1301 repairs the mismatched r611_end
# term (Jos: prefer repairing the branch over a new posture-specific
# hardcode) + the R1299b resolver cameras, ONE combined patch on the
# c198-run tree. THE c198 VERDICTS FIRST: (1) THE ARMSTART DOOR WON ITS
# CLASS - the never-started read RECURRED (FDF8=92180 armed at
# FE04=108995, drive parked 108933, act=0 pend=0 sched=0), the door
# fired ONCE, the first sector WAS consumed (fldsec #224 LBA 108995),
# FDF8 DRAINED TO 0, the drive advanced to 109004 - and the game walked
# DEEPER THAN EVER: into the 256xxx band running the full command
# ladder (cmds 01/07/0a/0c/0d/0e, seek=256934, pend=3, resp_n=3). The
# whole success chain receipted: started -> consumed -> completed ->
# advanced. (2) THE NEW FRONTIER WEDGE is the r611_end POSTURE ITSELF
# (schdd seq 264 BLOCKED x8: cmd=01 fe1c=7 FDF8=0 seek=109004
# FE04=109041 resp=0/1 A22C=4) - declined ONLY because the FE08 term
# fossilized at the c179-era field slot 0x8007F2F8 while the game now
# polls with FE08=80213680/80200680. THE REPAIR: loosen FE08 to a
# RAM-pointer sanity check; delivery runs through the EXISTING
# R612/R615 machinery (status 0x02, receipted accepted) + R613 event
# leg - ONE delivery per scheduled seq (sched must be 1 and the
# delivery retires it through the normal bookkeeping; unread responses
# preserved - the r611 branch primes only its own 1-byte answer in its
# own posture). THE R1300 movie-wait door (fe1c=10 class) NEVER FIRED
# this run - its class did not recur at the fuse; it stays guarded and
# reverts next cycle per the Jos recovery-experiment rule. THE CAMERAS
# (R1299b): [fdw] the FDF8 arm-context receipts (who armed 92180 - now
# also answerable for the 75796 arm at 109003), [actchg] the
# act/loaded transition trail, the serve ring, [freezedoss] the freeze
# dossier - all settle request identity by receipt. THE DECISIVE
# SUCCESS CHAIN (Jos): seq delivered -> guest consumes and acks ->
# wait exits (FE1C off 7) -> movie decoding or the next intended
# scene begins. FE1C movement alone is insufficient - the digest
# greps the full chain. IF DELIVERED BUT STILL PARKED: the blocked
# receipts name the remaining blocker vs an incorrect delivery BEFORE
# any revert. Base gate e00520d4 (the c198-run tree), patch sha
# e98e7e1b (sandbox-verified apply clean, parse clean, 0 -Wformat
# warnings), post-apply e80fc808, backup and restores read .bak1302.
# Bootstrap: fetch, SHA-256 gate, syntax gate, provenance copy,
# execute.

cd "$HOME/Downloads/xenolift" || exit 1

echo "===PRE200=== identity, tree drift gate (the c199-run tree e80fc808 - receipted by the c199 cycle own post-apply print)"
T=$(shasum -a 256 runtime/runtime.c | cut -d" " -f1)
echo "TREE_SHA=$T"
BASE="e80fc808cbddb44e69ec6372bea221d6ccc408d2535fdc70137515ffd9d3397b"
if [ "$T" != "$BASE" ]; then
  echo "DRIFT-FAILED: tree is not the c199-run tree e80fc808 - nothing run"
  exit 0
fi
T=$(shasum -a 256 runtime/runtime.c | cut -d" " -f1)
if [ "$T" != "$BASE" ]; then
...
echo "===PATCH200=== fetch, verify, apply the R1302 gettn+retire+era patch - marker (artifact-generated) + post-apply sha gates, FAIL EXPLICIT"
curl -sSL -m 30 -o /tmp/r1302_gettn.diff "https://base44.app/api/apps/6aa26c7ce0ac9a5ee05d2b95/files/mp/public/6aa26c7ce0ac9a5ee05d2b95/cf2e9a214_r1302_gettn.diff"
echo "CURL_RC=$?"
P=$(shasum -a 256 /tmp/r1302_gettn.diff | cut -d" " -f1)
echo "PATCH_SHA=$P"
if [ "$P" != "8b98e62737356b237a3cf5845f164427df3e33ad8d70f838842b277fee948eb6" ]; then
  echo "PATCH-VALID-FAILED: sha mismatch - nothing applied, nothing run"
  exit 0
fi
M1=$(grep -c "R1302" /tmp/r1302_gettn.diff)
M2=$(grep -c "RETIRE" /tmp/r1302_gettn.diff)
M3=$(grep -c "rspop01" /tmp/r1302_gettn.diff)
M4=$(grep -c "rspop13" /tmp/r1302_gettn.diff)
M5=$(grep -c "REPAIR-THE-BRANCH" /tmp/r1302_gettn.diff)
M6=$(grep -c "GetTN" /tmp/r1302_gettn.diff)
M7=$(grep -c "EXACTLY-ONCE" /tmp/r1302_gettn.diff)
M8=$(grep -c "era=" /tmp/r1302_gettn.diff)
echo "PATCH_MARKERS R1302=$M1 retire=$M2 rspop01=$M3 rspop13=$M4 repair=$M5 gettn=$M6 once=$M7 era=$M8"
if [ "$M1" != "11" ] || [ "$M2" != "1" ] || [ "$M3" != "1" ] || [ "$M4" != "1" ] || [ "$M5" != "1" ] || [ "$M6" != "3" ] || [ "$M7" != "1" ] || [ "$M8" != "3" ]; then
  echo "MARKER-FAILED: expected R1302=11 retire=1 rspop01=1 rspop13=1 repair=1 gettn=3 once=1 era=3 - nothing applied"
  exit 0
fi
cp -p runtime/runtime.c runtime/runtime.c.bak1302
if ! patch -p0 --dry-run runtime/runtime.c < /tmp/r1302_gettn.diff; then
  echo "DRYRUN-FAILED: patch does not apply cleanly - nothing changed"
  exit 0
fi
if ! patch -p0 runtime/runtime.c < /tmp/r1302_gettn.diff; then
  cp -p runtime/runtime.c.bak1302 runtime/runtime.c
  echo "APPLY-FAILED: restored backup, nothing run"
  exit 0
fi
N=$(shasum -a 256 runtime/runtime.c | cut -d" " -f1)
echo "NEW_SHA=$N"
if [ "$N" != "271b6bbffd4d742071da9af38abda832bcf1783bf20ad8bbf13e69739e504d8f" ]; then
  cp -p runtime/runtime.c.bak1302 runtime/runtime.c
  echo "POST-APPLY-FAILED: patched tree sha mismatch (expected 271b6bbf) - restored backup, nothing run"
  exit 0
fi
echo "PATCHED TREE VERIFIED (sandbox-parsed, sha-gated)"
echo "===FMTGATE200=== diagnostic-correctness build gate (Jos #1): -Wformat must be SILENT - diagnostics are implementation"
CC=$(command -v clang || command -v gcc)
if [ -z "$CC" ]; then echo "FMT-FAILED: no C compiler found - nothing run"; exit 0; fi
"$CC" -fsyntax-only -Wformat -Werror=format runtime/runtime.c 2>/tmp/fmt200.txt
FRC=$?
if [ "$FRC" != "0" ]; then echo "FMT-FAILED: format warnings present (rc=$FRC) - diagnostics are part of the implementation - nothing run"; head -8 /tmp/fmt200.txt; exit 0; fi
echo "FMT-GATE-PASSED: zero -Wformat warnings on the patched tree"

echo "===RUN200=== the 120s receipt run - alive-guarded captures, live-tail correlation"
...
echo "===STORM200=== THE GOVERNOR VERDICT - the storm must stay tamed (6.6MB class last run)"
...
echo "===CHAIN200=== THE DECISIVE CHAIN, line-numbered (Jos): the c199 receipts validated steps 1-3 for seq 128 (delivered -> consumed -> new commands + FE1C 10->11); this run receipts the GetTN link and beyond"
...
echo "===RESCUE200=== THE RESCUE CENSUS (Jos #5: which older recovery patches fired - retirement decisions come from these receipts)"
...
echo "===ERA200=== THE ERA CENSUS (requests tracked across their full lifetime, era-stamped)"
...
echo "===PROV200=== provenance"
LC_ALL=C grep -a -o "runtime build R[0-9]*" run.log | head -1
wc -l run.log
cp -p run.log run.log.c200
shasum -a 256 run.log run.log.c200 runtime/runtime.c
