#!/bin/bash
# c209 THE GETSTAT-POLL ACK REPAIR (R1310) - the c208 receipts
# name the NEXT missing transition, one state later than R1308
# repaired. (1) THE R1309 CAMERA VERDICT (the poison question
# ANSWERED): the chunk-entry r2 receipts are HEALTHY (80018EB0 =
# the CD command table pointer, or 00000000 on the benign paths -
# first 8 + every change, the whole run) - the c207 0x78-poison
# class DID NOT RECUR; the c208 first fault is a DIFFERENT class:
# faultctx target 0x3F002170 at pc=0x80041BA8 r31=0x80041C78
# (t=82s), then target 0x00200000 at pc=0x80041C68 with the
# STALE-CELL receipt "0x8004FB6C contains the target", 16
# recoveries, TRUE DEATH exit 99 at the same boot-family site
# (cur_fn=0x80019524). Per the lzss-runaway precedent the fault
# walk is DOWNSTREAM of the unanswered response loop - the
# revert-or-advance rule below decides. (2) R1308 VERDICT
# (receipted, the class CONVERTED): schdd delivery WIN seq=128
# (cmd=02 FDF8=2048) AND seq=129 (cmd=02) - BOTH cleared at the
# L2378 retire site - the seq-129 class that blocked x3 in c207
# now DELIVERS through the shared body. (3) THE NEW BLOCKED CLASS
# (the schdd terms, 8 receipts): the game ALIVE polling GetStat
# (cmd 0x01) every ~2s in its own protocol states - seq 130
# BLOCKED (cmd=01 fe1c=10 FDF8=2048 FE04=0 seek=1 resp=0/1),
# then seq 132 BLOCKED x8 (cmd=01 fe1c=11 FDF8=0 FE04=1 seek=1
# resp=0/1), every poll declined by every gate, the game own
# timeout walking the loop into the fault era. WHY the R1302
# arm declines both: it demands fe1c==10 (11 is out) and no
# unread response bytes (resp=0/1 = the command OWN 1-byte ack
# ARMED AT SET - the builder primes it at arrival - the same
# receipted principle as the c206 seq-129 correction, now
# demonstrated for the GetStat class: an armed-unpopped 1-byte
# envelope during a live poll loop is the PENDING DELIVERY, not
# stale unread data). (4) R1310 = THE ARM REPAIR: admit the
# RECEIPTED poll postures - cmd 0x01 + wait state 10 OR 11 + the
# single-byte own-ack envelope (n==1, pos<1) when the R1302 arm
# declined - delivered through the SAME shared body
# (cd_restore_pend re-primes the command own envelope; sched
# retired at L2378 = exactly-once per seq; INT3 + flag 578A6; NO
# R613 event, r611_end stays false - no seek-complete forgery).
# The data phase is untouched. REVERT next cycle if the game
# stays in the fe1c=11 wait after deliveries with NO new SETs and
# NO LBA consumption progress (the pre-registered rule). (5) The
# GetTN suppression receipts (958: "answer pending - no data
# event", the R310 wrong-worker guard) stay AS-IS: the guard is
# correct (a stale sector dressed as GetTN data dispatches the
# wrong worker - the cycle-58 lesson); the R1302 GetTN answer arm
# already covers the genuine TOC delivery. Also receipted: the
# movie re-request cadence now 4; fldsec multi-sector pace held
# (LBA 4 at t=34s); the screen held 100% nonblank full-screen
# through snap#12 then changed class at w95 (vram_nz 124455).
# Status per Jos wording: loading, decomposition, module
# installation, controller polling, and several response paths
# demonstrated; first interactive scene and sustained gameplay
# unproven; remaining blockers unknown. Base gate 73d51f76 (the
# c208-run tree, receipted by cycle 208 own NEW_SHA print), patch
# sha 53cfec8c (sandbox-verified: applies clean on the exact
# 73d51f76 tree, parse clean, ZERO -Wformat warnings), post-apply
# ccaffe8b9, backup/restore .bak1310. Bootstrap: fetch, SHA-256
# gate, syntax gate, provenance copy, execute.

cd "$HOME/Downloads/xenolift" || exit 1

echo "===PRE209=== identity, tree drift gate (the c204-run tree ecaab954 - receipted by the c204 cycle own NEW_SHA print)"
T=$(shasum -a 256 runtime/runtime.c | cut -d" " -f1)
echo "TREE_SHA=$T"
BASE="73d51f763c7b301e9aa45c865de8f97a0bcb86a72c54098110f6aa9e30ed690a"
if [ "$T" != "$BASE" ]; then
  echo "DRIFT-FAILED: tree is not the c208-run tree 73d51f76 - nothing run, nothing applied"
  exit 0
fi
echo "BASELINE_VERIFIED"

echo "===PATCH209=== fetch, verify, apply the R1305+R1306+R1307 measurement-corrected patch - marker (artifact-generated, line-count semantics) + post-apply sha gates, FAIL EXPLICIT"
curl -sSL -m 30 -o /tmp/r1310_getstat.diff "https://base44.app/api/apps/6aa26c7ce0ac9a5ee05d2b95/files/mp/public/6aa26c7ce0ac9a5ee05d2b95/47cc94796_r1310_getstat.diff"
echo "CURL_RC=$?"
P=$(shasum -a 256 /tmp/r1310_getstat.diff | cut -d" " -f1)
echo "PATCH_SHA=$P"
if [ "$P" != "53cfec8cbfdfe1abb9e0d4e1ae46ed3844629d4bb3a757eac22db758f49e0991" ]; then
  echo "PATCH-FAILED: patch sha mismatch - expected 53cfec8c - nothing applied"
  exit 0
fi
M1=$(grep -c "R1310" /tmp/r1310_getstat.diff)
M2=$(grep -c "seq-130/132" /tmp/r1310_getstat.diff)
M3=$(grep -c "OWN 1-byte ack ARMED AT SET" /tmp/r1310_getstat.diff)
M4=$(grep -c "REVERT next cycle" /tmp/r1310_getstat.diff)
echo "PATCH_MARKERS R1310=$M1 seq130=$M2 ownack=$M3 revert=$M4"
if [ "$M1" != "1" ] || [ "$M2" != "1" ] || [ "$M3" != "1" ] || [ "$M4" != "1" ]; then
  echo "MARKER-FAILED: expected R1310=1 seq130=1 ownack=1 revert=1 - nothing applied"
  exit 0
fi
cp -p runtime/runtime.c runtime/runtime.c.bak1310
if ! patch -p0 --dry-run runtime/runtime.c < /tmp/r1310_getstat.diff; then
  echo "DRYRUN-FAILED: patch does not apply clean - nothing changed"
  exit 0
fi
if ! patch -p0 runtime/runtime.c < /tmp/r1310_getstat.diff; then
  cp -p runtime/runtime.c.bak1310 runtime/runtime.c
  echo "APPLY-FAILED: restored backup - nothing run"
  exit 0
fi
N=$(shasum -a 256 runtime/runtime.c | cut -d" " -f1)
echo "NEW_SHA=$N"
if [ "$N" != "ccaffe8b9b7d39d78f2b4f3b0b294c3000e2b701c0e1538f4ec3f5a546aaaf59" ]; then
  echo "POST-APPLY-FAILED: patched tree sha mismatch (expected ccaffe8b9) - restored backup, nothing run"
  cp -p runtime/runtime.c.bak1310 runtime/runtime.c
  exit 0
fi
echo "PATCHED TREE VERIFIED (sandbox-parsed, sha-gated)"

echo "===PRESERVE209=== PRESERVE the existing run.log BEFORE the fresh run overwrites it (it holds the c204 receipts - the zero-blocked protocol run) - FAIL EXPLICIT"
TS=$(date +%Y%m%d_%H%M%S)
SNAP="runlog_preserve_c209_$TS"
if [ -f run.log ]; then
  mkdir -p "$SNAP"
...
echo "===FMTGATE209=== diagnostic-correctness build gate (Jos #1): -Wformat must be SILENT - diagnostics are implementation"
CC=$(command -v clang || command -v gcc)
if [ -z "$CC" ]; then echo "FMT-FAILED: no C compiler found - nothing run"; exit 0; fi
"$CC" -fsyntax-only -Wformat -Werror=format runtime/runtime.c 2>/tmp/fmt209.txt
FRC=$?
if [ "$FRC" != "0" ]; then echo "FMT-FAILED: format warnings present (rc=$FRC) - diagnostics are part of the implementation - nothing run"; head -8 /tmp/fmt209.txt; exit 0; fi
echo "FMT-GATE-PASSED: zero -Wformat warnings on the patched tree"

echo "===RUN209=== the 120s receipt run - alive-guarded captures, live-tail correlation"
...
echo "===STORM209=== THE GOVERNOR VERDICT - the storm must stay tamed (6.6MB class last run)"
...
echo "===PROVX209=== Jos step 1: BUILD PROVENANCE - connect the gated source sha to the executed binary (the unchanged R1291 banner is inconclusive)"
...
echo "===LOGDUP209=== Jos step 2: THE DUPLICATION QUESTION - logging pipeline duplicates output vs execution repeats (until resolved, tag counts are NOT reliable counts of deliveries, rescues, or restarts)"
...
echo "===FAULTX209=== Jos step 4: THE FIRST FAULT AND FINAL EXIT of THIS run (the truncated digest does not show them; RUNSH_RC=0 is the wrapper result, NOT proof of a clean guest exit)"
...
echo "===WAITBR209=== Jos step 3: THE WAIT-BRANCH RECEIPTS (LegacyCdDataWait actual return + caller branch operands in the same invocation - if ret==0 with FDFC==0+FE1C==0, the pre-movie-polling label is MISIDENTIFYING the stall)"
...
echo "===SYSACK209=== THE R1308 SYSTEM-AREA READ-ACK DELIVERIES (the seq-129/139 class - was blocked with terms in c205/c206)"
...
echo "===LZRUN209=== THE FIRST FAULT OF THE FINAL ERA (the lzss-runaway death walk - exit 99 was downstream of this)"
...
echo "===CDCSYNC209=== THE R1309 POISON-ENTRY RECEIPTS (the first fault of c207: WaitForVerticalRetrace called from LegacyCdCommandSync with a POISONED r2 - target 0x7786840C then 0x78787880, the literal 0x78 fill)"
...
grep -n "fetchcam" run.log | tail -8
echo "===CHAIN209=== THE CAUSAL TEST ON THE 8-BYTE DELTA (Jos c204: the truncation becomes a proven cause only if restoring the missing bytes removes the failure under comparable conditions) - this run receipts the tail delivery AND the failure discriminators"
