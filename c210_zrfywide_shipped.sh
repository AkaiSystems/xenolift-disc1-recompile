#!/bin/bash
# c210 THE MILESTONE VERDICT + THE ZRFY COMMAND-TERM WIDENING
# (R1311). (1) THE c209 RUN IS THE FIRST CLEAN FULL RUN OF THE
# PROJECT: exit status=0 (clean main-return via the b2exit
# epoch-return at the 120s fuse), ZERO fault lines (no faultctx,
# no segvdie, no crashkit - the fault-walk era is DEAD, it was
# downstream of the unanswered polls exactly as the lzss-runaway
# was downstream of the blocked read-ack), zero recoveries. (2)
# R1310 VERDICT: PASSES its pre-registered criteria - 14
# consecutive schdd WINs (seq 122-135, the cmd 02/01 alternation
# of the loader read+stat loop at native 2s pace), new SETs
# continuing (delivery-followed-by-another-SET = healthy, the
# WIN census criterion), the seq-130/132 classes that blocked x9
# in c208 CONVERTED, LBA 4 consumed at t=34s and LBA 0 at t=64s
# (the multi-sector pace held), the fetch chain streamed a NEW
# region (dest 8007AAF8 -> 8007CAF8 ascending at 0x800/sector,
# req words the HEALTHY 02010200 protocol class - not the c207
# garbage), the controller reader ran at scale (ReadController
# #159744, player=1, status=00, ID=40 - a REAL controller
# descriptor), the screen HELD 100 percent nonblank full-screen
# at snaps 9-12 through all witness windows (held-frame class
# d6b680b8, a complete image on screen at the fuse). R1310
# STAYS. (3) THE NEW NAMED POSTURE (the receipts, all late-era):
# cmd=01 fe1c=6 FE04=1 seek=1 FDF8=0 act=0 pend=0 sched=0
# resp_n=0 (fld2sig UNCOND x8, pumpcam rounds 76992+, fld2sig
# sched=0) - the ReadN DATA wait with the drive IDLE: the game
# re-reads the system area (LBA 0 done at t=64s, LBA 1 next),
# sits 56s at the posture to the fuse, ZERO serves. WHY the
# c171-era zrfY door sat silent: its gate demands cd_last_cmd==
# 0x09, but the R1310 deliveries made the loader poll loop LIVE -
# its GetStat (cmd 01) polls now DELIVER between the read issue
# and the data wait, so cd_last_cmd reads 01 at the wait - terms
# written for the pre-R1310 era (the c346 audit lesson). (4)
# R1311 = THE COMMAND-TERM WIDENING: admit cmd 01 at the
# RECEIPTED data-wait state fe1c=6 ONLY (the c124 ladder: cmd 09
# -> 6 = ReadN issued, awaiting DATA - the GetStat is the
# loader interleave, not a new request); cmd 01 at the other
# fe1c states stays EXCLUDED. All other door terms unchanged:
# two-tick confirm, FDF8 0->2048 heal, FE04 stamp, read_active +
# same-poll cd_data_load (the drive own machinery), stale-sched
# clear, force-deliver INT1, budget 40. REVERT if the serve
# fires but FDF8 never drains or no new SET follows (the
# pre-registered rule). (5) The poison classes stay silent
# (one 0x8000008C store receipt, module-era taintw prints only -
# the c207/c208 fault class did not recur; the R1309 camera
# receipts healthy r2s all run). Status per Jos wording:
# loading, decomposition, module installation, controller
# polling, and a widening set of response paths DEMONSTRATED -
# plus a NEW first: a complete clean 120s run with a full-screen
# image held at the fuse; first interactive scene and sustained
# gameplay UNPROVEN; remaining blockers UNKNOWN (the current
# head: the LBA-1-class system-area data-phase serve). Base
# gate ccaffe8b9 (the c209-run tree, receipted by cycle 209 own
# NEW_SHA print), patch sha 368b5e23 (sandbox-verified: applies
# clean on the exact ccaffe8b9 tree, parse clean, ZERO -Wformat
# warnings), post-apply 17dbe0c2, backup/restore .bak1311.
# Bootstrap: fetch, SHA-256 gate, syntax gate, provenance copy,
# execute.

cd "$HOME/Downloads/xenolift" || exit 1

echo "===PRE210=== identity, tree drift gate (the c204-run tree ecaab954 - receipted by the c204 cycle own NEW_SHA print)"
T=$(shasum -a 256 runtime/runtime.c | cut -d" " -f1)
echo "TREE_SHA=$T"
BASE="ccaffe8b9b7d39d78f2b4f3b0b294c3000e2b701c0e1538f4ec3f5a546aaaf59"
if [ "$T" != "$BASE" ]; then
  echo "DRIFT-FAILED: tree is not the c209-run tree ccaffe8b9 - nothing run, nothing applied"
  exit 0
fi
echo "BASELINE_VERIFIED"

echo "===PATCH210=== fetch, verify, apply the R1305+R1306+R1307 measurement-corrected patch - marker (artifact-generated, line-count semantics) + post-apply sha gates, FAIL EXPLICIT"
curl -sSL -m 30 -o /tmp/r1311_zrfywide.diff "https://base44.app/api/apps/6aa26c7ce0ac9a5ee05d2b95/files/mp/public/6aa26c7ce0ac9a5ee05d2b95/6121b1ef2_r1311_zrfywide.diff"
echo "CURL_RC=$?"
P=$(shasum -a 256 /tmp/r1311_zrfywide.diff | cut -d" " -f1)
echo "PATCH_SHA=$P"
if [ "$P" != "368b5e236d0c3b7651a55b5ba610f18f57b5dad2d1e2d088b70c26b16c22e9bd" ]; then
  echo "PATCH-FAILED: patch sha mismatch - expected 368b5e23 - nothing applied"
  exit 0
fi
M1=$(grep -c "R1311" /tmp/r1311_zrfywide.diff)
M2=$(grep -c "command-term widening" /tmp/r1311_zrfywide.diff)
M3=$(grep -c "REVERT if the serve fires" /tmp/r1311_zrfywide.diff)
M4=$(grep -c "c346 audit lesson" /tmp/r1311_zrfywide.diff)
echo "PATCH_MARKERS R1311=$M1 wide=$M2 revert=$M3 audit=$M4"
if [ "$M1" != "1" ] || [ "$M2" != "1" ] || [ "$M3" != "1" ] || [ "$M4" != "1" ]; then
  echo "MARKER-FAILED: expected R1311=1 wide=1 revert=1 audit=1 - nothing applied"
  exit 0
fi
cp -p runtime/runtime.c runtime/runtime.c.bak1311
if ! patch -p0 --dry-run runtime/runtime.c < /tmp/r1311_zrfywide.diff; then
  echo "DRYRUN-FAILED: patch does not apply clean - nothing changed"
  exit 0
fi
if ! patch -p0 runtime/runtime.c < /tmp/r1311_zrfywide.diff; then
  cp -p runtime/runtime.c.bak1311 runtime/runtime.c
  echo "APPLY-FAILED: restored backup - nothing run"
  exit 0
fi
N=$(shasum -a 256 runtime/runtime.c | cut -d" " -f1)
echo "NEW_SHA=$N"
if [ "$N" != "17dbe0c24114e596226530ec66e3088b23a96521d05d38cb5071b5ae7c8d077f" ]; then
  echo "POST-APPLY-FAILED: patched tree sha mismatch (expected 17dbe0c2) - restored backup, nothing run"
  cp -p runtime/runtime.c.bak1311 runtime/runtime.c
  exit 0
fi
echo "PATCHED TREE VERIFIED (sandbox-parsed, sha-gated)"

echo "===PRESERVE210=== PRESERVE the existing run.log BEFORE the fresh run overwrites it (it holds the c204 receipts - the zero-blocked protocol run) - FAIL EXPLICIT"
TS=$(date +%Y%m%d_%H%M%S)
SNAP="runlog_preserve_c210_$TS"
if [ -f run.log ]; then
  mkdir -p "$SNAP"
...
echo "===FMTGATE210=== diagnostic-correctness build gate (Jos #1): -Wformat must be SILENT - diagnostics are implementation"
...
echo "===RUN210=== the 120s receipt run - alive-guarded captures, live-tail correlation"
...
echo "===STORM210=== THE GOVERNOR VERDICT - the storm must stay tamed (6.6MB class last run)"
...
echo "===PROVX210=== Jos step 1: BUILD PROVENANCE - connect the gated source sha to the executed binary (the unchanged R1291 banner is inconclusive)"
...
echo "===LOGDUP210=== Jos step 2: THE DUPLICATION QUESTION - logging pipeline duplicates output vs execution repeats (until resolved, tag counts are NOT reliable counts of deliveries, rescues, or restarts)"
...
echo "===FAULTX210=== Jos step 4: THE FIRST FAULT AND FINAL EXIT of THIS run (the truncated digest does not show them; RUNSH_RC=0 is the wrapper result, NOT proof of a clean guest exit)"
...
echo "===WAITBR210=== Jos step 3: THE WAIT-BRANCH RECEIPTS (LegacyCdDataWait actual return + caller branch operands in the same invocation - if ret==0 with FDFC==0+FE1C==0, the pre-movie-polling label is MISIDENTIFYING the stall)"
...
echo "===SYSACK210=== THE R1308 SYSTEM-AREA READ-ACK DELIVERIES (the seq-129/139 class - was blocked with terms in c205/c206)"
...
echo "===LZRUN210=== THE FIRST FAULT OF THE FINAL ERA (the lzss-runaway death walk - exit 99 was downstream of this)"
...
echo "===CDCSYNC210=== THE R1309 POISON-ENTRY RECEIPTS (the first fault of c207: WaitForVerticalRetrace called from LegacyCdCommandSync with a POISONED r2 - target 0x7786840C then 0x78787880, the literal 0x78 fill)"
...
grep -n "fetchcam" run.log | tail -8
echo "===CHAIN210=== THE CAUSAL TEST ON THE 8-BYTE DELTA (Jos c204: the truncation becomes a proven cause only if restoring the missing bytes removes the failure under comparable conditions) - this run receipts the tail delivery AND the failure discriminators"
