#!/bin/bash
# c211 THE TRANSFER-IDENTITY LEDGER (R1312) - Jos's c210 order:
# establish the outstanding-transfer chain BEFORE any serve rests on
# last_cmd evidence. (1) THE DESIGN WEAKNESS ACCEPTED: last_cmd is
# NOT transfer state - an interleaved GetStat (cmd 01) changes
# last_cmd without identifying whether a read remains OUTSTANDING;
# FE1C=6 + FE04=1 + an idle drive identify only a POSTURE, and
# FDF8=0 leaves the outstanding-byte requirement UNRESOLVED - so the
# R1311 widened arm (already dispatched as the c210 narrow test with
# its pre-registered revert) is judged by the STRICT criterion: one
# correct sector consumed by the IDENTIFIED request followed by the
# intended loader transition - NEVER by the door firing alone. If
# the serve fires without identified-request consumption, R1311
# REVERTS next cycle regardless of any counter movement. (2) R1312
# = THE EVIDENCE RUN (cameras + ledger state, NO behavior change):
# a per-transfer ledger arms at Setloc (0x02 re-aim: a DIFFERENT LBA
# retires the prior record, same-LBA re-issue keeps it) and at
# ReadN/ReadS (06/09: stamps dest=FE08 + len=FDF8 - the length the
# game ITSELF arms, already receipted by [fdw] with the writer fn);
# the STATUS FAMILY (01/13/14) is PASSIVE - it only counts the
# intervene (gst), NEVER retires; Stop (08) retires; the DRIVE-OWN
# completion (FDF8 drained + read_active cleared + FIFO empty)
# retires at the poll camera. The new [xfer] DATA-WAIT camera prints
# whether the fe1c=6 wait is OWNED by a named transfer (live=1 +
# matching lba + a receipted no-retire chain) or UNOWNED (live=0 -
# last_cmd evidence alone, which must NOT serve). (3) WHERE THE
# REQUESTED LENGTH LIVES: at issue time the game writes it into
# FDF8 (the [fdw] ARM-CONTEXT receipts name the writer fn, the
# requested LBA and the drive posture); at the c209 posture FDF8=0 -
# the ledger receipts WHICH: (a) armed-then-drained = the wait is
# POST-completion (the game waiter never saw completion - a delivery
# defect, not a serve defect), or (b) never-armed = the length lives
# only in the game's own request state and the serve must take its
# authorization from the ledger, not from FDF8. The receipts decide
# which; the durable fix (identity-authorized delivery) follows from
# them. (4) THE MILESTONE QUALIFIED (Jos correction accepted): the
# c209 exit-0 came at EXACTLY t=120s = the RUN_BUDGET_S=120 fuse
# returning through the b2exit epoch-return path - the run's TIME
# LIMIT, not natural completion; clean exit + fourteen WINs prove
# DELIVERY PROGRESS only; first interactive scene and sustained
# gameplay remain UNPROVEN; remaining blockers UNKNOWN.
cd "$HOME/Downloads/xenolift" || exit 1

echo "===PRE211=== identity, tree drift gate (the c204-run tree ecaab954 - receipted by the c204 cycle own NEW_SHA print)"
T=$(shasum -a 256 runtime/runtime.c | cut -d" " -f1)
echo "TREE_SHA=$T"
BASE="17dbe0c24114e596226530ec66e3088b23a96521d05d38cb5071b5ae7c8d077f"
if [ "$T" != "$BASE" ]; then
  echo "DRIFT-FAILED: tree is not the c210-run tree 17dbe0c2 - nothing run, nothing applied"
  exit 0
fi
echo "BASELINE_VERIFIED"

echo "===PATCH211=== fetch, verify, apply the R1305+R1306+R1307 measurement-corrected patch - marker (artifact-generated, line-count semantics) + post-apply sha gates, FAIL EXPLICIT"
curl -sSL -m 30 -o /tmp/r1312_xfer.diff "https://base44.app/api/apps/6aa26c7ce0ac9a5ee05d2b95/files/mp/public/6aa26c7ce0ac9a5ee05d2b95/3325320e4_r1312_xfer.diff"
echo "CURL_RC=$?"
P=$(shasum -a 256 /tmp/r1312_xfer.diff | cut -d" " -f1)
echo "PATCH_SHA=$P"
if [ "$P" != "14464c50953f4a3ff0c1df364e90a67cd25861cc719987104fd5cd939440eacf" ]; then
  echo "PATCH-FAILED: patch sha mismatch - expected 14464c50 - nothing applied"
  exit 0
fi
M1=$(grep -c "R1312" /tmp/r1312_xfer.diff)
M2=$(grep -c "xfer" /tmp/r1312_xfer.diff)
M3=$(grep -c "transfer-identity" /tmp/r1312_xfer.diff)
M4=$(grep -c "DATA-WAIT" /tmp/r1312_xfer.diff)
echo "PATCH_MARKERS R1312=$M1 xfer=$M2 ident=$M3 wait=$M4"
if [ "$M1" != "9" ] || [ "$M2" != "6" ] || [ "$M3" != "2" ] || [ "$M4" != "1" ]; then
  echo "MARKER-FAILED: expected R1312=9 xfer=6 ident=2 wait=1 - nothing applied"
  exit 0
fi
cp -p runtime/runtime.c runtime/runtime.c.bak1312
if ! patch -p0 --dry-run runtime/runtime.c < /tmp/r1312_xfer.diff; then
  echo "DRYRUN-FAILED: patch does not apply clean - nothing changed"
  exit 0
fi
if ! patch -p0 runtime/runtime.c < /tmp/r1312_xfer.diff; then
  cp -p runtime/runtime.c.bak1312 runtime/runtime.c
  echo "APPLY-FAILED: restored backup - nothing run"
  exit 0
fi
N=$(shasum -a 256 runtime/runtime.c | cut -d" " -f1)
echo "NEW_SHA=$N"
if [ "$N" != "799522a66541b59dc617d00f88c0a3d64105ab917978bfc70f5112b88e0cd3af" ]; then
  echo "POST-APPLY-FAILED: patched tree sha mismatch (expected 799522a6) - restored backup, nothing run"
  cp -p runtime/runtime.c.bak1312 runtime/runtime.c
  exit 0
fi
echo "PATCHED TREE VERIFIED (sandbox-parsed, sha-gated)"

echo "===PRESERVE211=== PRESERVE the existing run.log BEFORE the fresh run overwrites it (it holds the c204 receipts - the zero-blocked protocol run) - FAIL EXPLICIT"
TS=$(date +%Y%m%d_%H%M%S)
SNAP="runlog_preserve_c211_$TS"
if [ -f run.log ]; then
  mkdir -p "$SNAP"
...
echo "===FMTGATE211=== diagnostic-correctness build gate (Jos #1): -Wformat must be SILENT - diagnostics are implementation"
...
echo "===RUN211=== the 120s receipt run - alive-guarded captures, live-tail correlation"
...
echo "===STORM211=== THE GOVERNOR VERDICT - the storm must stay tamed (6.6MB class last run)"
...
echo "===XFER211=== THE R1312 TRANSFER-IDENTITY LEDGER (Jos c210 order: establish the outstanding-transfer chain BEFORE serving on last_cmd evidence)"
...
echo "--- the c210-run zrfY verdict FROM THE PRESERVED LOG (Jos criterion: identified-request consumption, not door-fire alone)"
P211=$(ls -dt runlog_preserve_c211_* 2>/dev/null | head -1)
if [ -n "$P211" ] && [ -f "$P211/run.log" ]; then
  grep -n "\[zrfY\]" "$P211/run.log" | head -12
  echo "c210-run zrfY fires: $(grep -c "\[zrfY\]" "$P211/run.log" || true)"
  echo "c210-run fe1c=6 idle data-wait receipts (fld2sig UNCOND): $(grep -c "UNCOND sched=0" "$P211/run.log" || true)"
else
  echo "PRESERVE-EXTRACT-FAILED: c211 preserve dir not found"
fi

echo "===PROVX211=== Jos step 1: BUILD PROVENANCE - connect the gated source sha to the executed binary (the unchanged R1291 banner is inconclusive)"
...
echo "===LOGDUP211=== Jos step 2: THE DUPLICATION QUESTION - logging pipeline duplicates output vs execution repeats (until resolved, tag counts are NOT reliable counts of deliveries, rescues, or restarts)"
...
echo "===FAULTX211=== Jos step 4: THE FIRST FAULT AND FINAL EXIT of THIS run (the truncated digest does not show them; RUNSH_RC=0 is the wrapper result, NOT proof of a clean guest exit)"
...
echo "===WAITBR211=== Jos step 3: THE WAIT-BRANCH RECEIPTS (LegacyCdDataWait actual return + caller branch operands in the same invocation - if ret==0 with FDFC==0+FE1C==0, the pre-movie-polling label is MISIDENTIFYING the stall)"
...
echo "===SYSACK211=== THE R1308 SYSTEM-AREA READ-ACK DELIVERIES (the seq-129/139 class - was blocked with terms in c205/c206)"
...
echo "===LZRUN211=== THE FIRST FAULT OF THE FINAL ERA (the lzss-runaway death walk - exit 99 was downstream of this)"
...
echo "===CDCSYNC211=== THE R1309 POISON-ENTRY RECEIPTS (the first fault of c207: WaitForVerticalRetrace called from LegacyCdCommandSync with a POISONED r2 - target 0x7786840C then 0x78787880, the literal 0x78 fill)"
...
grep -n "fetchcam" run.log | tail -8
echo "===CHAIN211=== THE CAUSAL TEST ON THE 8-BYTE DELTA (Jos c204: the truncation becomes a proven cause only if restoring the missing bytes removes the failure under comparable conditions) - this run receipts the tail delivery AND the failure discriminators"
