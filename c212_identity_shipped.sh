#!/bin/bash
# c212 THE IDENTITY-AUTHORIZED SERVE (R1313) - the durable fix
# Jos named, built on the c211 ledger receipts. (1) THE c211
# VERDICT: the R1312 ledger answered the ownership question with
# receipts - the fe1c=6 data wait at seek=1 is OWNED by a NAMED
# transfer (live=1 lba=1 seq=128 gst=13 age=54s; live=1:8 live=0:0),
# 13 intervening status commands counted and NEVER retiring, no
# Stop, no drive-own completion, and len=0 AT ARM TIME - the
# requested length was NEVER armed in FDF8, it lives in the game
# own request descriptor (the never-armed case, distinguishing
# (a) armed-then-drained = post-completion delivery defect from
# (b) never-armed = request-path serve defect - case (b) is
# RECEIPTED). The c210 preserved-log verdict: the R1311 widened
# arm fired ZERO serves all run (zrfY fires: 0) - last_cmd evidence
# alone correctly served nothing. (2) R1313 = THE DURABLE FIX (Jos:
# preserve transfer identity across status commands and use it to
# authorize delivery): the zrfY system-band door RETIRES the R1311
# command-term widening entirely - it now fires ONLY when the
# ledger names a LIVE transfer whose LBA matches the drive seek
# cell in the system band (cd_x_live && cd_x_lba == cd_seek_lba
# && seek < 150), never on last_cmd. The serve is the proven
# composite: FDF8 0->2048 one-sector length heal, FE04 stamp,
# read_active + same-poll cd_data_load of the NAMED LBA,
# stale-sched clear, force-deliver INT1, budget 40, two-tick
# confirm. A new [xfer] R1313 serve receipt prints the identity
# chain at the fire (lba, origin seq, gst intervene count, age,
# FE08 dest, and last_cmd explicitly NOT the authorizer). (3)
# SUCCESS CRITERION (Jos, verbatim): one correct sector consumed
# by the IDENTIFIED request - receipted by the existing R1312
# drive-own completion retire (FDF8 drained + read_active cleared
# + FIFO empty) for THIS lba - followed by the intended loader
# transition (new SET / fe1c exit / new xfer ARM) - NEVER the door
# firing alone. REVERT if the serve fires but no completion retire
# and no new SET follow (pre-registered). (4) Unchanged: the
# c211 new fault class (target 0x00200000 at pc=0x8004B694
# r31=0x8004B5E4, stale-cells 8001A3D0/8001CE8C/8001D010 +
# family, exit 99 after 16 declined recoveries, death at the boot
# family 80019524) is receipted and NOT touched - the identity
# serve targets the receipted owned wait; if the fault class
# recurs post-serve it becomes the next first-fault per the Jos
# ladder. Status: loading, decomposition, module installation,
# controller polling, several response paths, and now TRANSFER
# IDENTITY demonstrated; first interactive scene and sustained
# gameplay UNPROVEN; remaining blockers UNKNOWN.
cd "$HOME/Downloads/xenolift" || exit 1

echo "===PRE212=== identity, tree drift gate (the c204-run tree ecaab954 - receipted by the c204 cycle own NEW_SHA print)"
T=$(shasum -a 256 runtime/runtime.c | cut -d" " -f1)
echo "TREE_SHA=$T"
BASE="799522a66541b59dc617d00f88c0a3d64105ab917978bfc70f5112b88e0cd3af"
if [ "$T" != "$BASE" ]; then
  echo "DRIFT-FAILED: tree is not the c211-run tree 799522a6 - nothing run, nothing applied"
  exit 0
fi
echo "BASELINE_VERIFIED"

echo "===PATCH212=== fetch, verify, apply the R1305+R1306+R1307 measurement-corrected patch - marker (artifact-generated, line-count semantics) + post-apply sha gates, FAIL EXPLICIT"
curl -sSL -m 30 -o /tmp/r1313_identity.diff "https://base44.app/api/apps/6aa26c7ce0ac9a5ee05d2b95/files/mp/public/6aa26c7ce0ac9a5ee05d2b95/e27c40d4d_r1313_identity.diff"
echo "CURL_RC=$?"
P=$(shasum -a 256 /tmp/r1313_identity.diff | cut -d" " -f1)
echo "PATCH_SHA=$P"
if [ "$P" != "cf525be732b87f5b07fdf3823bfa7b7366962a052f9e82c515e9e595b4f0da63" ]; then
  echo "PATCH-FAILED: patch sha mismatch - expected cf525be7 - nothing applied"
  exit 0
fi
M1=$(grep -c "R1313" /tmp/r1313_identity.diff)
M2=$(grep -c "identity-authorized" /tmp/r1313_identity.diff)
M3=$(grep -c "cd_x_live == 1" /tmp/r1313_identity.diff)
M4=$(grep -c "NOT the authorizer" /tmp/r1313_identity.diff)
echo "PATCH_MARKERS R1313=$M1 ident=$M2 xlive=$M3 auth=$M4"
if [ "$M1" != "2" ] || [ "$M2" != "1" ] || [ "$M3" != "1" ] || [ "$M4" != "1" ]; then
  echo "MARKER-FAILED: expected R1313=2 ident=1 xlive=1 auth=1 - nothing applied"
  exit 0
fi
cp -p runtime/runtime.c runtime/runtime.c.bak1313
if ! patch -p0 --dry-run runtime/runtime.c < /tmp/r1313_identity.diff; then
  echo "DRYRUN-FAILED: patch does not apply clean - nothing changed"
  exit 0
fi
if ! patch -p0 runtime/runtime.c < /tmp/r1313_identity.diff; then
  cp -p runtime/runtime.c.bak1313 runtime/runtime.c
  echo "APPLY-FAILED: restored backup - nothing run"
  exit 0
fi
N=$(shasum -a 256 runtime/runtime.c | cut -d" " -f1)
echo "NEW_SHA=$N"
if [ "$N" != "d48ed004758bb35f6ebd5481e1461fc2bd329c87d82bb0821dad6ad20e085153" ]; then
  echo "POST-APPLY-FAILED: patched tree sha mismatch (expected d48ed004) - restored backup, nothing run"
  cp -p runtime/runtime.c.bak1313 runtime/runtime.c
  exit 0
fi
echo "PATCHED TREE VERIFIED (sandbox-parsed, sha-gated)"

echo "===PRESERVE212=== PRESERVE the existing run.log BEFORE the fresh run overwrites it (it holds the c211 receipts - the owned-wait ledger run) - FAIL EXPLICIT"
...
echo "===FMTGATE212=== diagnostic-correctness build gate (Jos #1): -Wformat must be SILENT - diagnostics are implementation"
...
echo "===RUN212=== the 120s receipt run - alive-guarded captures, live-tail correlation"
...
echo "===STORM212=== THE GOVERNOR VERDICT - the storm must stay tamed (6.6MB class last run)"
...
echo "===XFER212=== THE R1312 TRANSFER-IDENTITY LEDGER (Jos c210 order: establish the outstanding-transfer chain BEFORE serving on last_cmd evidence)"
...
echo "===PROVX212=== Jos step 1: BUILD PROVENANCE - connect the gated source sha to the executed binary (the unchanged R1291 banner is inconclusive)"
...
echo "===LOGDUP212=== Jos step 2: THE DUPLICATION QUESTION - logging pipeline duplicates output vs execution repeats (until resolved, tag counts are NOT reliable counts of deliveries, rescues, or restarts)"
...
echo "===FAULTX212=== Jos step 4: THE FIRST FAULT AND FINAL EXIT of THIS run (the truncated digest does not show them; RUNSH_RC=0 is the wrapper result, NOT proof of a clean guest exit)"
...
echo "===WAITBR212=== Jos step 3: THE WAIT-BRANCH RECEIPTS (LegacyCdDataWait actual return + caller branch operands in the same invocation - if ret==0 with FDFC==0+FE1C==0, the pre-movie-polling label is MISIDENTIFYING the stall)"
...
echo "===SYSACK212=== THE R1308 SYSTEM-AREA READ-ACK DELIVERIES (the seq-129/139 class - was blocked with terms in c205/c206)"
...
echo "===LZRUN212=== THE FIRST FAULT OF THE FINAL ERA (the lzss-runaway death walk - exit 99 was downstream of this)"
...
echo "===CDCSYNC212=== THE R1309 POISON-ENTRY RECEIPTS (the first fault of c207: WaitForVerticalRetrace called from LegacyCdCommandSync with a POISONED r2 - target 0x7786840C then 0x78787880, the literal 0x78 fill)"
...
grep -n "fetchcam" run.log | tail -8
echo "===CHAIN212=== THE CAUSAL TEST ON THE 8-BYTE DELTA (Jos c204: the truncation becomes a proven cause only if restoring the missing bytes removes the failure under comparable conditions) - this run receipts the tail delivery AND the failure discriminators"
