#!/bin/bash
# c213 THE PUMP-SITE IDENTITY SERVE (R1314) - the c212 verdict
# receipts: the owned wait held to the fuse and the R1313 write-site
# door NEVER fired. During the terminal fe1c=6 wait the game issues NO
# new command writes (cd_last_cmd=01 stale from BEFORE the wait), so a
# write-site door never evaluates its terms - while the pump-site
# camera observed the owned posture (live=1 lba=1 seq=128 gst=13,
# age 54s, live=1:8 live=0:0) every 64 rounds to the 120s fuse. The
# c178-era class (R1246 c103: the fetch PsyQ-table sequence never
# engages the read machinery) explains why no write ever comes.
# R1314 = the SAME identity-authorized composite (FDF8 one-sector
# heal + FE04 stamp + read_active + stale-sched clear + force-deliver
# INT1, budget 40, two-tick confirm) RELOCATED to the pump site that
# actually observes the owned wait, keyed on the SAME identity terms
# (cd_x_live && cd_x_lba == cd_seek_lba && seek < 150), NEVER on
# last_cmd. The record len is stamped at serve so the EXISTING R1312
# drive-own completion retire (FDF8 drained + read_active cleared +
# FIFO empty) receipts the consumption for THIS lba. SUCCESS (Jos
# criterion, unchanged): one correct sector consumed by the IDENTIFIED
# request + the intended loader transition (new SET / fe1c exit / new
# ARM) - NEVER the door firing alone; REVERT if fire w/o retire.
# The R1313 write-site door stays (harmless: if a command write ever
# visits during a wait, identity terms apply there too). Unchanged:
# the c211/c212 fault class receipted untouched. Status per Jos
# wording: loading, decompression, module installation, controller
# polling, several response paths, and TRANSFER IDENTITY demonstrated;
# first interactive scene and sustained gameplay UNPROVEN; remaining
# blockers UNKNOWN. Base gate d48ed004 (the c212-run tree, receipted
# by cycle 212 own NEW_SHA print), patch sha e000b596
# (sandbox-verified: applies clean on the exact d48ed004 tree, parse
# clean, ZERO -Wformat warnings), post-apply 915af54f, backup/restore
# .bak1314. Harness 16/16 against a disposable replica of the exact
# tree incl. drift-rejection. Bootstrap: fetch, SHA-256 gate, syntax
# gate, provenance copy, execute.

echo "===PRE213=== identity, tree drift gate (the c204-run tree ecaab954 - receipted by the c204 cycle own NEW_SHA print)"
T=$(shasum -a 256 runtime/runtime.c | cut -d" " -f1)
echo "TREE_SHA=$T"
BASE="d48ed004758bb35f6ebd5481e1461fc2bd329c87d82bb0821dad6ad20e085153"
if [ "$T" != "$BASE" ]; then
...
echo "===PATCH213=== fetch, verify, apply the R1305+R1306+R1307 measurement-corrected patch - marker (artifact-generated, line-count semantics) + post-apply sha gates, FAIL EXPLICIT"
curl -sSL -m 30 -o /tmp/r1314_pumpserve.diff "https://base44.app/api/apps/6aa26c7ce0ac9a5ee05d2b95/files/mp/public/6aa26c7ce0ac9a5ee05d2b95/64215ec18_r1314_pumpserve.diff"
echo "CURL_RC=$?"
P=$(shasum -a 256 /tmp/r1314_pumpserve.diff | cut -d" " -f1)
echo "PATCH_SHA=$P"
if [ "$P" != "e000b5963f3eb5d107c59e6658a8813291556a400aa26331118df3ec1e8f3733" ]; then
  echo "PATCH-FAILED: patch sha mismatch - expected e000b596 - nothing applied"
  exit 0
fi
M1=$(grep -c "R1314" /tmp/r1314_pumpserve.diff)
M2=$(grep -c "pump-site" /tmp/r1314_pumpserve.diff)
M3=$(grep -c "xferP" /tmp/r1314_pumpserve.diff)
M4=$(grep -c "R1314 pump-site identity serve fire" /tmp/r1314_pumpserve.diff)
echo "PATCH_MARKERS R1314=$M1 pumpsite=$M2 xferP=$M3 servefire=$M4"
if [ "$M1" != "4" ] || [ "$M2" != "4" ] || [ "$M3" != "1" ] || [ "$M4" != "1" ]; then
  echo "MARKER-FAILED: expected R1314=4 pumpsite=4 xferP=1 servefire=1 - nothing applied"
  exit 0
fi
cp -p runtime/runtime.c runtime/runtime.c.bak1314
if ! patch -p0 --dry-run runtime/runtime.c < /tmp/r1314_pumpserve.diff; then
  echo "DRYRUN-FAILED: patch does not apply clean - nothing changed"
  exit 0
fi
if ! patch -p0 runtime/runtime.c < /tmp/r1314_pumpserve.diff; then
...
echo "NEW_SHA=$N"
if [ "$N" != "915af54f9e64fd20701706b70ea4d9fa86fb6c9f636385aea6d577791d9c8e68" ]; then
...
echo "===PRESERVE213=== PRESERVE the existing run.log BEFORE the fresh run overwrites it (it holds the c212 receipts - the R1313-silent owned-wait run) - FAIL EXPLICIT"
...
echo "===FMTGATE213=== diagnostic-correctness build gate (Jos #1): -Wformat must be SILENT - diagnostics are implementation"
...
echo "===RUN213=== the 120s receipt run - alive-guarded captures, live-tail correlation"
...
echo "===STORM213=== THE GOVERNOR VERDICT - the storm must stay tamed (6.6MB class last run)"
...
echo "===XFER213=== THE R1312 TRANSFER-IDENTITY LEDGER (Jos c210 order: establish the outstanding-transfer chain BEFORE serving on last_cmd evidence)"
...
echo "--- the R1314 PUMP-SITE identity serve fires + heals + consumption (THE c213 verdict)"
grep -n "R1314" run.log | head -20
echo "--- the consumption + transition census"
echo "R1314 fires: $(grep -c "R1314 pump-site identity serve fire" run.log || true)"
echo "R1312 drive-own completion retires: $(grep -c "DRIVE-OWN COMPLETION retire" run.log || true)"
echo "new ARM after first serve: $(grep -c "xfer\] R1312 ARM" run.log || true)"
...
echo "===PROVX213=== Jos step 1: BUILD PROVENANCE - connect the gated source sha to the executed binary (the unchanged R1291 banner is inconclusive)"
...
echo "===LOGDUP213=== Jos step 2: THE DUPLICATION QUESTION - logging pipeline duplicates output vs execution repeats (until resolved, tag counts are NOT reliable counts of deliveries, rescues, or restarts)"
...
echo "===FAULTX213=== Jos step 4: THE FIRST FAULT AND FINAL EXIT of THIS run (the truncated digest does not show them; RUNSH_RC=0 is the wrapper result, NOT proof of a clean guest exit)"
...
echo "===WAITBR213=== Jos step 3: THE WAIT-BRANCH RECEIPTS (LegacyCdDataWait actual return + caller branch operands in the same invocation - if ret==0 with FDFC==0+FE1C==0, the pre-movie-polling label is MISIDENTIFYING the stall)"
...
echo "===SYSACK213=== THE R1308 SYSTEM-AREA READ-ACK DELIVERIES (the seq-129/139 class - was blocked with terms in c205/c206)"
...
echo "===LZRUN213=== THE FIRST FAULT OF THE FINAL ERA (the lzss-runaway death walk - exit 99 was downstream of this)"
...
echo "===CDCSYNC213=== THE R1309 POISON-ENTRY RECEIPTS (the first fault of c207: WaitForVerticalRetrace called from LegacyCdCommandSync with a POISONED r2 - target 0x7786840C then 0x78787880, the literal 0x78 fill)"
...
grep -n "fetchcam" run.log | tail -8
echo "===CHAIN213=== THE CAUSAL TEST ON THE 8-BYTE DELTA (Jos c204: the truncation becomes a proven cause only if restoring the missing bytes removes the failure under comparable conditions) - this run receipts the tail delivery AND the failure discriminators"
