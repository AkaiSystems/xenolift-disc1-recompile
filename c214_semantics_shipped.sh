#!/bin/bash
# c214 THE COMMAND-SEMANTICS AUDIT + THREE-IDENTITY LEDGER + SECTOR
# CORRECTNESS (R1314) - the Jos PSX-SPX correction. (1) COMMAND
# SEMANTICS: 0x09 is PAUSE (INT3 ack then INT2 completion; it ENDS
# reading), NOT ReadS - the prior label booked Pause commands as read
# arms (the c212/c213 "ARM (ReadS) lba=0 dest=0 len=0" records at cmd
# 09 were Pauses). 0x0D is SETFILTER, NOT a read - the c190/c191
# read-inference is RETRACTED. 0x1B is the real ReadS; 0x06 ReadN.
# The runtime may treat 09 as a read elsewhere (retaining a cancelled
# transfer, waiting for the wrong completion) - this cycle RECEIPTS,
# it does not yet repair: a raw command-byte audit prints every byte
# written to the command register (raw-vs-alias decided), the ledger
# arms ONLY at 06/1B with dest+len stamped at issue, PAUSE (09) now
# RETIRES the stream like Stop (distinct class, INT2-unreceipted
# noted), Setfilter is passive. (2) THE THREE IDENTITIES (c213 order):
# POSITION intent (Setloc: lba+age only, never serve authority; a
# re-aim retires a live stream but arms nothing), ACTIVE READ STREAM
# (armed ONLY at ReadN/ReadS; passive through status commands; retired
# by drive-own completion / Stop / Pause / re-aim), GUEST BUFFER
# REQUEST (the game own descriptor: req59EF8 words + the live FE08
# dest cell, printed at the DATA-WAIT posture). A retained live=1 flag
# is NEVER permission to inject another sector - the R1313 door now
# requires a genuine stream (len and dest both nonzero). (3) SECTOR
# CORRECTNESS (the standing priority): every serve now prints its
# identity (lba + MSF header + payload sum/xor signature) - the same
# signature across advancing fetch dests = the same bytes repeatedly
# delivered (wrong-sector delivery); advancing signatures = real
# streaming. The c213 run receipted the fetch chain advancing deeper
# than ever (seeks 13-16, FE04 matching, req words advancing) - this
# cycle receipts WHICH sectors those fetches actually received.
# Decisive chains grep the RAW stream (duplicate WIN lines remain in
# the processed log). Status per Jos wording: loading, decompression,
# module installation, controller polling, several response paths
# demonstrated; first interactive scene and sustained gameplay
# UNPROVEN; remaining blockers UNKNOWN. Base gate 915af54f (the
# c213-run tree, receipted by cycle 213 own NEW_SHA print), patch sha
# 588cbc98 (sandbox-verified: applies clean on the exact 915af54f
# tree, parse clean, ZERO -Wformat warnings), post-apply 205cbcec,
# backup/restore .bak1314. Harness 16/16 against a disposable replica
# of the exact tree incl. drift-rejection. Bootstrap: fetch, SHA-256
# gate, syntax gate, provenance copy, execute.
echo "===PRE214=== identity, tree drift gate (the c204-run tree ecaab954 - receipted by the c204 cycle own NEW_SHA print)"
T=$(shasum -a 256 runtime/runtime.c | cut -d" " -f1)
echo "TREE_SHA=$T"
BASE="915af54f9e64fd20701706b70ea4d9fa86fb6c9f636385aea6d577791d9c8e68"
if [ "$T" != "$BASE" ]; then
  echo "DRIFT-FAILED: tree is not the c213-run tree 915af54f - nothing run, nothing applied"
  exit 0
fi
echo "BASELINE_VERIFIED"

echo "===PATCH214=== fetch, verify, apply the R1305+R1306+R1307 measurement-corrected patch - marker (artifact-generated, line-count semantics) + post-apply sha gates, FAIL EXPLICIT"
curl -sSL -m 30 -o /tmp/r1315_semantics.diff "https://base44.app/api/apps/6aa26c7ce0ac9a5ee05d2b95/files/mp/public/6aa26c7ce0ac9a5ee05d2b95/66d8cf587_r1315_semantics_v2.diff"
echo "CURL_RC=$?"
P=$(shasum -a 256 /tmp/r1315_semantics.diff | cut -d" " -f1)
echo "PATCH_SHA=$P"
if [ "$P" != "588cbc98c5c4c70f75ab4baadc8113329f00ceae35c4263642b5ca7b0d847ab2" ]; then
  echo "PATCH-FAILED: patch sha mismatch - expected 588cbc98 - nothing applied"
  exit 0
fi
M1=$(grep -c "R1314" /tmp/r1315_semantics.diff)
M2=$(grep -c "POS intent" /tmp/r1315_semantics.diff)
M3=$(grep -c "STREAM-ARM" /tmp/r1315_semantics.diff)
M5=$(grep -c "PAUSE (09)" /tmp/r1315_semantics.diff)
M6=$(grep -c "SETFILTER" /tmp/r1315_semantics.diff)
M4=$(grep -c "sector serve" /tmp/r1315_semantics.diff)
echo "PATCH_MARKERS R1314=$M1 pos=$M2 stream=$M3 sector=$M4 pause=$M5 setfilter=$M6"
if [ "$M1" != "19" ] || [ "$M2" != "1" ] || [ "$M3" != "1" ] || [ "$M4" != "1" ] || [ "$M5" != "2" ] || [ "$M6" != "1" ]; then
  echo "MARKER-FAILED: expected R1314=19 pos=1 stream=1 sector=1 pause=2 setfilter=1 - nothing applied"
  exit 0
fi
cp -p runtime/runtime.c runtime/runtime.c.bak1314
if ! patch -p0 --dry-run runtime/runtime.c < /tmp/r1315_semantics.diff; then
  echo "DRYRUN-FAILED: patch does not apply clean - nothing changed"
  exit 0
fi
if ! patch -p0 runtime/runtime.c < /tmp/r1315_semantics.diff; then
  cp -p runtime/runtime.c.bak1314 runtime/runtime.c
  echo "APPLY-FAILED: restored backup - nothing run"
  exit 0
fi
N=$(shasum -a 256 runtime/runtime.c | cut -d" " -f1)
echo "NEW_SHA=$N"
if [ "$N" != "205cbcec5e5d212539e88f545f765d3bc3f4de5681ae3f987643091c9ac83a07" ]; then
  echo "POST-APPLY-FAILED: patched tree sha mismatch (expected 915af54f) - restored backup, nothing run"
  cp -p runtime/runtime.c.bak1314 runtime/runtime.c
  exit 0
fi
echo "PATCHED TREE VERIFIED (sandbox-parsed, sha-gated)"

echo "===PRESERVE214=== PRESERVE the existing run.log BEFORE the fresh run overwrites it (it holds the c213 receipts - the R1314-silent trajectory-shift run) - FAIL EXPLICIT"
...
echo "===FMTGATE214=== diagnostic-correctness build gate (Jos #1): -Wformat must be SILENT - diagnostics are implementation"
...
echo "===RUN214=== the 120s receipt run - alive-guarded captures, live-tail correlation"
...
echo "===STORM214=== THE GOVERNOR VERDICT - the storm must stay tamed (6.6MB class last run)"
...
echo "===XFER214=== THE R1312 TRANSFER-IDENTITY LEDGER (Jos c210 order: establish the outstanding-transfer chain BEFORE serving on last_cmd evidence)"
...
echo "===PROVX214=== Jos step 1: BUILD PROVENANCE - connect the gated source sha to the executed binary (the unchanged R1291 banner is inconclusive)"
...
echo "===LOGDUP214=== Jos step 2: THE DUPLICATION QUESTION - logging pipeline duplicates output vs execution repeats (until resolved, tag counts are NOT reliable counts of deliveries, rescues, or restarts)"
...
echo "===FAULTX214=== Jos step 4: THE FIRST FAULT AND FINAL EXIT of THIS run (the truncated digest does not show them; RUNSH_RC=0 is the wrapper result, NOT proof of a clean guest exit)"
...
echo "===WAITBR214=== Jos step 3: THE WAIT-BRANCH RECEIPTS (LegacyCdDataWait actual return + caller branch operands in the same invocation - if ret==0 with FDFC==0+FE1C==0, the pre-movie-polling label is MISIDENTIFYING the stall)"
...
echo "===SYSACK214=== THE R1308 SYSTEM-AREA READ-ACK DELIVERIES (the seq-129/139 class - was blocked with terms in c205/c206)"
...
echo "===LZRUN214=== THE FIRST FAULT OF THE FINAL ERA (the lzss-runaway death walk - exit 99 was downstream of this)"
...
echo "===CDCSYNC214=== THE R1309 POISON-ENTRY RECEIPTS (the first fault of c207: WaitForVerticalRetrace called from LegacyCdCommandSync with a POISONED r2 - target 0x7786840C then 0x78787880, the literal 0x78 fill)"
...
grep -n "fetchcam" run.log | tail -8
echo "===CHAIN214=== THE CAUSAL TEST ON THE 8-BYTE DELTA (Jos c204: the truncation becomes a proven cause only if restoring the missing bytes removes the failure under comparable conditions) - this run receipts the tail delivery AND the failure discriminators"
