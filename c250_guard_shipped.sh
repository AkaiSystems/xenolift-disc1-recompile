#!/bin/bash
# c250_arena_guard.sh - THE R1325 DISPATCHER ARENA WALK GUARD (one
...
checks = [("R1325", data.count(b"R1325"), 4),
          ("arenaseed-tag", data.count(b"[arenaseed]"), 2),
          ("walk-key", data.count(b"0x8003223Cu"), 2),
          ("seed-writes", data.count(b"xenolift_mem_write32(0x800CC26Cu"), 1),
