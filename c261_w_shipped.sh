#!/bin/bash
# c261_zrfg2_widen.sh (v2 of c260) - THE R1303 ONE-TERM WIDEN of zrfG2
# (the sgarm
# v2 CORRECTION (marker gate caught it, nothing written, tree intact
# 33402d7f): the FAIL-EXPLICIT context printed the TRUE zrfG2 gate -
# exactly the c191 design (r1290_stuck/budget=12, !read_active,
# pend==0, cmd==01, seek>=100000, FE04==seek, FDF8 0<x<300000,
# partition guard cd_data_n<2048) - and the PATCHED-CONTEXT shows the
# widen landing byte-exact. ONLY the R1303-added marker want was
# wrong (before+2; the insert adds exactly ONE R1303 mention; got=1).
# The c135 class, 4th occurrence. v2: want=before+1. Everything else
# passed (cmd01 unchanged=18, cmd02 23->24).
...
checks = [("cmd01-unchanged", c01, before01),
          ...
          ("R1303-added", cR, data.count(b"R1303") + 1)]
