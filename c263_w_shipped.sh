#!/bin/bash
# c263_zrfg_widen.sh - THE R1305 ONE-TERM WIDEN of zrfG (the sgarm
# precedent, 5th application; one term, runtime.c). THE c261 RECEIPTS:
...
if [ "$SS" != "$EXPECT" ]; then
  if grep -q "R1305" "$SRC" 2>/dev/null; then
    echo "GATE-ALT: tree already carries R1305 (patch landed before a crash) - IDEMPOTENT mode: no patch, evidence run only; tree=$SS"
    ALREADY=1
  else
    echo "GATE-FAILED: runtime.c is neither the R1303 tree 2b2eb134 nor an R1305 tree - refusing a foreign tree"; exit 0
  fi
fi
ALREADY=${ALREADY:-0}
...
if [ "$ALREADY" -eq 1 ]; then
  echo "===PATCH263=== SKIPPED (tree already carries R1305) - parse + evidence run only"
else
echo "===PATCH263=== the R1305 zrfG cmd widen (python, binary-safe, FAIL EXPLICIT)"
python3 - <<'PYEOF' || { echo "PATCH-FAILED - nothing written, no run"; exit 1; }
import hashlib, sys
data = open("runtime/runtime.c","rb").read()
if b"R1305" in data and b"R1303" in data and data.count(b"cd_last_cmd == 0x02u") >= 2:
    print("IDEMPOTENT-SKIP: R1305-era widen already present - no edit made"); sys.exit(0)
...
beforeR5 = data.count(b"R1305")
print("PRIOR counts: cmd01=%d cmd02=%d R1305=%d" % (before01, before02, beforeR5))
...
new = b"(cd_last_cmd == 0x01u || cd_last_cmd == 0x02u) /* R1305 (c263): the c258 f15 wedge holds last_cmd=02; the A22C-site sibling (R1303) never evaluated - this era polls the status site; one-term widen, sgarm precedent; safety terms unchanged */"
...
checks = [("cmd01-unchanged", data2.count(b"cd_last_cmd == 0x01u"), before01),
          ...
          ("R1305-added", data2.count(b"R1305"), beforeR5 + 1)]
...
echo "===ZRFG263=== the door receipts (did the status-site widen fire?)"
grep -n "zrfG\]\|R1285\|R1305" run.log 2>/dev/null | head -12
...
echo "===C263DONE=== zrfG widen cycle complete - verdict from these receipts; REVERT = restore $P/runtime.c.pre"
