#!/bin/bash
# c265_zrfg_widen_v3.sh (v3 of the c265 ship) - THE R1329 zrfG WIDEN,
# CLEAN RE-APPLY after the c264 MISPLACEMENT
# v3: the gate accepts BOTH the misplaced tree (revert path) and the
# ALREADY-CLEAN baseline 2b2eb134 (the c265 v1 revert landed; its
# patch then failed on the marker trip, leaving nothing applied -
# the c266 run receipted this state and refused; skip the revert) (a new c135-class variant: correct marker
...
CLEAN="2b2eb13460b03ed8fd127a6a834f81bc884d00820397c44eab4b5de7440a9bfe"
SKIP_REVERT=0
if [ "$SS" = "$CLEAN" ]; then
  echo "GATE-CLEAN: tree already at the reverted baseline 2b2eb134 (the c265 v1 revert landed) - skipping the revert step"
  SKIP_REVERT=1
elif [ "$SS" != "$CUR" ]; then
  echo "GATE-FAILED: runtime.c is neither the c264 tree 2eab69dc nor the clean baseline 2b2eb134 - refusing"; exit 0
fi
...
RS="$SS"
echo "REVERT-SKIPPED (already clean)"
fi
if grep -q "R1328" "$SRC"; then echo "C265-FAILED: R1328 still present"; exit 1; fi
...
new = (b"(cd_last_cmd == 0x01u || cd_last_cmd == 0x02u) /* R1329 (c265): the c258 f15 wedge holds last_cmd=02 post-Setloc; zrfG2's A22C site ran cold (its own widen never evaluated), the status site is the poller; one-term widen, sgarm precedent; safety terms unchanged */\n"
       b"                && cd_seek_lba >= 100000u")
