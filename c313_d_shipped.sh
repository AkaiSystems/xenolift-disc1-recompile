#!/bin/bash
# c313_nearcam_v2_reship.sh - R1343 (reship: corrected marker gates): the change-detection decline
...
mkdir -p "patch_c313_$TS"
cp -p "$SRC" "patch_c313_$TS/runtime.c.pre"
echo "PRESERVED: patch_c313_$TS/runtime.c.pre"
python3 - <<'PYEOF'
...
echo "NEW_SHA=$SS2"
clang -fsyntax-only -std=gnu99 "$SRC" 2>/tmp/c313_parse.txt; PRC=$?
echo "PARSE_RC=$PRC"
if [ "$PRC" != "0" ]; then echo "PARSE-FAILED - reverting"; head -5 /tmp/c313_parse.txt; cp -p "patch_c313_$TS/runtime.c.pre" "$SRC"; exit 0; fi
echo "PARSE-OK"
echo "===RUN313=== the R1343 build (reship), 120s budget, first-fault stop"
XENOLIFT_FIRSTFAULT_STOP=1 RUN_BUDGET_S=120 bash run.sh > /tmp/run_full_c313.txt 2>&1; RRC=$?
echo "RUN_RC=$RRC"
WDIR="witness_c313_$TS"; mkdir -p "$WDIR"
for f in run.log run.log.d; do [ -f "$f" ] && cp -p "$f" "$WDIR/$f"; done
cp -p /tmp/run_full_c313.txt "$WDIR/run_full.txt"
echo "WITNESS=$WDIR"
LOG="run.log"; [ -s "$LOG" ] || LOG="run.log.d"
echo "===NEAR313=== the v2 decline receipts (all eras incl. the tail)"
grep -n "stalecd-near" "$LOG" | head -44
echo "===MISM313=== the seekmism verdicts"
grep -n "seekmism" "$LOG" | tail -20
echo "===DISCARD313=== discard + assist fires"
grep -n "stalecd\] R1341" "$LOG" | head -6
grep -n "FIELD-SEEK" "$LOG" | head -10
echo "===PASS3313=== the pass-3 story"
grep -n "\[chg\] FE1C" "$LOG" | tail -10
grep -n "cdw02" "$LOG" | tail -4
echo "===FLOW313=== READ ISSUED + fld2sig tail"
grep -c "READ ISSUED" "$LOG"
grep -n "fld2sig" "$LOG" | tail -4
echo "===FAULT313=== fault census"
...
echo "===VIS313=== visuals + run tail"
grep -n "nonblank" "$LOG" | tail -3
tail -6 /tmp/run_full_c313.txt
grep -n "rungasp" "$LOG" | tail -2
echo "===C313DONE=== R1343 reship run complete - the tail refusing term is receipted or the path proven never-reached"
