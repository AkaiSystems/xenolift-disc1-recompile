#!/bin/bash
# c314_nearcam_v2_reship2.sh - R1343 (reship 2: off-by-one block length fixed, hardened boundary check): the change-detection decline
...
mkdir -p "patch_c314_$TS"
cp -p "$SRC" "patch_c314_$TS/runtime.c.pre"
echo "PRESERVED: patch_c314_$TS/runtime.c.pre"
python3 - <<'PYEOF'
...
if lines[i0+15].strip() != b"}": ok_struct = False
if b"fassist_budget" not in lines[i0+16]: ok_struct = False
print("STRUCT_OK=%s (4-point boundary: i0+6 open, i0+14 inner close, i0+15 outer close, i0+16 anchor)" % ok_struct)
if not ok_struct:
    print("PATCH-FAILED - v1 block boundary check failed - nothing written, no run")
    raise SystemExit(0)
...
lines[i0:i0+16] = new_block
post = b"\n".join(lines)
checks = [(b"R1343", 2), (b"seekmism", 2), (b"R1342", 1),
...
echo "NEW_SHA=$SS2"
clang -fsyntax-only -std=gnu99 "$SRC" 2>/tmp/c314_parse.txt; PRC=$?
echo "PARSE_RC=$PRC"
if [ "$PRC" != "0" ]; then echo "PARSE-FAILED - reverting"; head -5 /tmp/c314_parse.txt; cp -p "patch_c314_$TS/runtime.c.pre" "$SRC"; exit 0; fi
echo "PARSE-OK"
echo "===RUN314=== the R1343 build (reship), 120s budget, first-fault stop"
XENOLIFT_FIRSTFAULT_STOP=1 RUN_BUDGET_S=120 bash run.sh > /tmp/run_full_c314.txt 2>&1; RRC=$?
echo "RUN_RC=$RRC"
WDIR="witness_c314_$TS"; mkdir -p "$WDIR"
for f in run.log run.log.d; do [ -f "$f" ] && cp -p "$f" "$WDIR/$f"; done
cp -p /tmp/run_full_c314.txt "$WDIR/run_full.txt"
echo "WITNESS=$WDIR"
LOG="run.log"; [ -s "$LOG" ] || LOG="run.log.d"
echo "===NEAR314=== the v2 decline receipts (all eras incl. the tail)"
grep -n "stalecd-near" "$LOG" | head -44
echo "===MISM314=== the seekmism verdicts"
grep -n "seekmism" "$LOG" | tail -20
echo "===DISCARD314=== discard + assist fires"
grep -n "stalecd\] R1341" "$LOG" | head -6
grep -n "FIELD-SEEK" "$LOG" | head -10
echo "===PASS3314=== the pass-3 story"
grep -n "\[chg\] FE1C" "$LOG" | tail -10
grep -n "cdw02" "$LOG" | tail -4
echo "===FLOW314=== READ ISSUED + fld2sig tail"
grep -c "READ ISSUED" "$LOG"
grep -n "fld2sig" "$LOG" | tail -4
echo "===FAULT314=== fault census"
...
echo "===VIS314=== visuals + run tail"
grep -n "nonblank" "$LOG" | tail -3
tail -6 /tmp/run_full_c314.txt
grep -n "rungasp" "$LOG" | tail -2
echo "===C314DONE=== R1343 reship run complete - the tail refusing term is receipted or the path proven never-reached"
