#!/bin/bash
# c339_epochidle.sh - R1358: the genuine-answer epoch ends at
# the kernel's own idle return (FE1C==0), not pend==0-at-drain.
# THE c338 CONVICTION: the SetLoc drain fires while pend is
# STILL 0, moments BEFORE the ReadN's INT3 arms - the pend-only
# clear exits the epoch before the ReadN enters it. The epoch
# boundary must be FE1C==0 + pend==0 (true kernel idle).
# THIS CYCLE: (1) preserve pre; (2) R1358 - the slots-drain
# clear gains the FE1C==0 term + a budgeted holding receipt;
# (3) parse gate; (4) 120s run; (5) census.
set -u
cd "$HOME/Downloads/xenolift" || { echo "C339-FAILED: xenolift dir missing"; exit 1; }
export LC_ALL=C
...
mkdir -p "patch_c339_$TS"
cp -p "$SRC" "patch_c339_$TS/runtime.c.pre"
echo "PRESERVED: patch_c339_$TS/runtime.c.pre"
python3 - <<'PYEOF'
...
pre_checks = [(b"R1358", 0),
              ...
              (b"g_r1351_genuine_answer", 6), (b"L18405", 1), (b"cd_pending == 0u", 1)]
...
post = b"\n".join(lines)
checks = [(b"R1358", 3), (b"R1357", 3), (b"R1355", 2), (b"[wgen]", 3),
          (b"g_r1351_genuine_answer", 6), (b"L18405", 1), (b"cd_pending == 0u", 2),
          (b"xenolift_mem_read32(0x8004FE1Cu)", 2),
...
print("PATCH-APPLIED (R1358 epoch-idle flag clear)")
PYEOF
SS2=$(shasum -a 256 "$SRC" | cut -d" " -f1)
echo "NEW_SHA=$SS2"
clang -fsyntax-only -std=gnu99 "$SRC" 2>/tmp/c339_parse.txt; PRC=$?
echo "PARSE_RC=$PRC"
if [ "$PRC" != "0" ]; then echo "PARSE-FAILED - reverting"; head -8 /tmp/c339_parse.txt; cp -p "patch_c339_$TS/runtime.c.pre" "$SRC"; exit 0; fi
echo "PARSE-OK"
echo "===RUN339=== the R1358 build, 120s budget, first-fault stop"
XENOLIFT_FIRSTFAULT_STOP=1 RUN_BUDGET_S=120 bash run.sh > /tmp/run_full_c339.txt 2>&1; RRC=$?
echo "RUN_RC=$RRC"
WDIR="witness_c339_$TS"; mkdir -p "$WDIR"
for f in run.log run.log.d; do [ -f "$f" ] && cp -p "$f" "$WDIR/$f"; done
cp -p /tmp/run_full_c339.txt "$WDIR/run_full.txt"
echo "WITNESS=$WDIR"
LOG="run.log"; [ -s "$LOG" ] || LOG="run.log.d"
echo "===HOLD339=== the holding receipts (the R1358 epoch evidence)"
grep -n "R1358" "$LOG" | head -16
echo "===READN339=== the ReadN INT3 consumption (the R1358 verdict)"
...
echo "===POPS339=== tail census post-ReadN"
if [ -n "$RN" ]; then
...
echo "===FE1C339=== the FE1C walk after the serve"
grep -n "\[chg\] FE1C" "$LOG" | tail -10
echo "===OUTCOME339=== cmdtl + data posture + mvloop"
grep -n "cmdtl" "$LOG" | tail -4
grep -n "fld2sig" "$LOG" | tail -3
grep -n "mvloop" "$LOG" | tail -2
echo "===FAULT339=== fault census"
...
echo "===VIS339=== run tail + gpu census"
grep -n "rungasp" "$LOG" | tail -2
grep -n "gpufin\|gp0_words" "$LOG" | tail -2
echo "===C339DONE=== R1358 run complete - the epoch-idle verdict comes from these receipts"
