#!/bin/bash
# c340_epochidle2.sh - R1358 reship: epoch-idle flag clear with
# corrected marker gates.
...
set -u
cd "$HOME/Downloads/xenolift" || { echo "C340-FAILED: xenolift dir missing"; exit 1; }
export LC_ALL=C
...
mkdir -p "patch_c340_$TS"
cp -p "$SRC" "patch_c340_$TS/runtime.c.pre"
echo "PRESERVED: patch_c340_$TS/runtime.c.pre"
python3 - <<'PYEOF'
...
print("PATCH-APPLIED (R1358 epoch-idle flag clear, c340 reship)")
PYEOF
SS2=$(shasum -a 256 "$SRC" | cut -d" " -f1)
echo "NEW_SHA=$SS2"
clang -fsyntax-only -std=gnu99 "$SRC" 2>/tmp/c340_parse.txt; PRC=$?
echo "PARSE_RC=$PRC"
if [ "$PRC" != "0" ]; then echo "PARSE-FAILED - reverting"; head -8 /tmp/c340_parse.txt; cp -p "patch_c340_$TS/runtime.c.pre" "$SRC"; exit 0; fi
echo "PARSE-OK"
echo "===RUN340=== the R1358 build, 120s budget, first-fault stop"
XENOLIFT_FIRSTFAULT_STOP=1 RUN_BUDGET_S=120 bash run.sh > /tmp/run_full_c340.txt 2>&1; RRC=$?
echo "RUN_RC=$RRC"
WDIR="witness_c340_$TS"; mkdir -p "$WDIR"
for f in run.log run.log.d; do [ -f "$f" ] && cp -p "$f" "$WDIR/$f"; done
cp -p /tmp/run_full_c340.txt "$WDIR/run_full.txt"
echo "WITNESS=$WDIR"
LOG="run.log"; [ -s "$LOG" ] || LOG="run.log.d"
echo "===HOLD340=== the holding receipts (the R1358 epoch evidence)"
grep -n "R1358" "$LOG" | head -16
echo "===READN340=== the ReadN INT3 consumption (the R1358 verdict)"
...
echo "===POPS340=== tail census post-ReadN"
if [ -n "$RN" ]; then
...
echo "===FE1C340=== the FE1C walk after the serve"
grep -n "\[chg\] FE1C" "$LOG" | tail -10
echo "===OUTCOME340=== cmdtl + data posture + mvloop"
grep -n "cmdtl" "$LOG" | tail -4
grep -n "fld2sig" "$LOG" | tail -3
grep -n "mvloop" "$LOG" | tail -2
echo "===FAULT340=== fault census"
...
echo "===VIS340=== run tail + gpu census"
grep -n "rungasp" "$LOG" | tail -2
grep -n "gpufin\|gp0_words" "$LOG" | tail -2
echo "===C340DONE=== R1358 run complete - the epoch-idle verdict comes from these receipts"
