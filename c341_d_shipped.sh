#!/bin/bash
# c341_epochidle3.sh - R1358 reship 2: epoch-idle flag clear with
...
set -u
cd "$HOME/Downloads/xenolift" || { echo "C341-FAILED: xenolift dir missing"; exit 1; }
export LC_ALL=C
...
mkdir -p "patch_c341_$TS"
cp -p "$SRC" "patch_c341_$TS/runtime.c.pre"
echo "PRESERVED: patch_c341_$TS/runtime.c.pre"
python3 - <<'PYEOF'
...
print("PATCH-APPLIED (R1358 epoch-idle flag clear, c341 reship)")
PYEOF
SS2=$(shasum -a 256 "$SRC" | cut -d" " -f1)
echo "NEW_SHA=$SS2"
clang -fsyntax-only -std=gnu99 "$SRC" 2>/tmp/c341_parse.txt; PRC=$?
echo "PARSE_RC=$PRC"
if [ "$PRC" != "0" ]; then echo "PARSE-FAILED - reverting"; head -8 /tmp/c341_parse.txt; cp -p "patch_c341_$TS/runtime.c.pre" "$SRC"; exit 0; fi
echo "PARSE-OK"
echo "===RUN341=== the R1358 build, 120s budget, first-fault stop"
XENOLIFT_FIRSTFAULT_STOP=1 RUN_BUDGET_S=120 bash run.sh > /tmp/run_full_c341.txt 2>&1; RRC=$?
echo "RUN_RC=$RRC"
WDIR="witness_c341_$TS"; mkdir -p "$WDIR"
for f in run.log run.log.d; do [ -f "$f" ] && cp -p "$f" "$WDIR/$f"; done
cp -p /tmp/run_full_c341.txt "$WDIR/run_full.txt"
echo "WITNESS=$WDIR"
...
echo "===FE1C341=== the FE1C walk after the serve"
...
echo "===C341DONE=== R1358 run complete - the epoch-idle verdict comes from these receipts"
