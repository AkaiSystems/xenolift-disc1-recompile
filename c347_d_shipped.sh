#!/bin/bash
# c347_pausefix.sh - R1359: PAUSE (09) ENDS READING at the
...
set -u
cd "$HOME/Downloads/xenolift" || { echo "C347-FAILED: xenolift dir missing"; exit 1; }
export LC_ALL=C
...
mkdir -p "patch_c347_$TS"
cp -p "$SRC" "patch_c347_$TS/runtime.c.pre"
echo "PRESERVED: patch_c347_$TS/runtime.c.pre"
python3 - <<'PYEOF'
...
if len(i1) != 1 or len(i2) != 1:
    ...
    raise SystemExit(0)
new1 = b"    } else if (cmd == 0x06u || cmd == 0x1Bu) { /* R1359 (c347): 0x09 REMOVED - PSX-SPX Pause ENDS reading; the R163 ReadS reclass retracted here too (R1314 fixed the stream layer only); 0x1B is the real ReadS. ReadN/ReadS: data will be"
new2 = b"    } else if (cmd == 0x08u || cmd == 0x09u) { /* R1359 (c347): Stop and Pause BOTH end reading per PSX-SPX - the R163-era 0x09 reclass retracted at the command site */"
...
post = b"\n".join(lines)
checks = [(b"R1359", 4), (b"cmd == 0x06u || cmd == 0x1Bu", 1),
          (b"cmd == 0x08u || cmd == 0x09u", 1),
...
echo "NEW_SHA=$SS2"
clang -fsyntax-only -std=gnu99 "$SRC" 2>/tmp/c347_parse.txt; PRC=$?
echo "PARSE_RC=$PRC"
if [ "$PRC" != "0" ]; then echo "PARSE-FAILED - reverting"; head -8 /tmp/c347_parse.txt; cp -p "patch_c347_$TS/runtime.c.pre" "$SRC"; exit 0; fi
echo "PARSE-OK"
echo "===RUN347=== the R1359 build, 120s budget, first-fault stop"
XENOLIFT_FIRSTFAULT_STOP=1 RUN_BUDGET_S=120 bash run.sh > /tmp/run_full_c347.txt 2>&1; RRC=$?
echo "RUN_RC=$RRC"
WDIR="witness_c347_$TS"; mkdir -p "$WDIR"
for f in run.log run.log.d; do [ -f "$f" ] && cp -p "$f" "$WDIR/$f"; done
cp -p /tmp/run_full_c347.txt "$WDIR/run_full.txt"
echo "WITNESS=$WDIR"
...
echo "===FE1C347=== the FE1C walk in the f15 era"
...
echo "===C347DONE=== R1359 run complete - the f15-era verdict comes from these receipts"
