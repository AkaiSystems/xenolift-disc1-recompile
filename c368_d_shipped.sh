#!/bin/bash
# c368_schedentry4.sh - R1365 reship (c365 gate-fix: POST R1365=3).
...
set -u
cd "$HOME/Downloads/xenolift" || { echo "C368-FAILED: xenolift dir missing"; exit 1; }
export LC_ALL=C
...
mkdir -p "patch_c368_$TS"
cp -p "$SRC" "patch_c368_$TS/runtime.c.pre"
echo "PRESERVED: patch_c368_$TS/runtime.c.pre"
python3 - <<'PYEOF'
...
for tok, want in [(b"R1365", 0), (b"[fsent]", 0), (b"a == 0x800286CCu", 4), (OUTER, 1), (BUSY, 1)]:
    c = data.count(tok)
...
checks = [(b"R1365", 3), (b"[fsent] R1365", 1), (b"R1365 (c365)", 2), (OUTER, 0),
          ...
          (b"g_r1351_genuine_answer", 8), (b"a == 0x800286CCu", 5), (b"a == 0x800286CCu && g_r1351_genuine_answer", 2),
...
open("/tmp/c368_patched.flag", "w").write("ok")
print("PATCH-APPLIED (R1365 scheduled-read entry arm)")
PYEOF
if [ ! -f /tmp/c368_patched.flag ]; then echo "PATCH-FAILED - exiting without parse or run"; exit 0; fi
SS2=$(shasum -a 256 "$SRC" | cut -d" " -f1)
echo "NEW_SHA=$SS2"
clang -fsyntax-only -std=gnu99 "$SRC" 2>/tmp/c368_parse.txt; PRC=$?
echo "PARSE_RC=$PRC"
if [ "$PRC" != "0" ]; then echo "PARSE-FAILED - reverting"; head -8 /tmp/c368_parse.txt; cp -p "patch_c368_$TS/runtime.c.pre" "$SRC"; exit 0; fi
echo "PARSE-OK"
echo "===RUN368=== the R1365 build, 120s budget, first-fault stop"
XENOLIFT_FIRSTFAULT_STOP=1 RUN_BUDGET_S=120 bash run.sh > /tmp/run_full_c368.txt 2>&1; RRC=$?
echo "RUN_RC=$RRC"
WDIR="witness_c368_$TS"; mkdir -p "$WDIR"
for f in run.log run.log.d; do [ -f "$f" ] && cp -p "$f" "$WDIR/$f"; done
cp -p /tmp/run_full_c368.txt "$WDIR/run_full.txt"
echo "WITNESS=$WDIR"
LOG="run.log"; [ -s "$LOG" ] || LOG="run.log.d"
echo "===FSENT368=== the scheduled-read entry receipts"
grep -n "fsent" "$LOG" | head -10
echo "===ZRF368=== the zrf door receipts (did the stream start?)"
grep -n "zrf" "$LOG" | grep -v zrfY | head -8
grep -c "zrf. zero-length read" "$LOG"
echo "===SECT368=== the file-14 sectors (LBA 108933+) - the stream census"
grep -c "sector LBA 1089" "$LOG"
grep -n "sector LBA 1089" "$LOG" | head -6
grep -n "zrfm" "$LOG" | head -4
echo "===LZSS368=== the unpack receipts (sane word0 at 801DD680?)"
grep -n "lzss-t" "$LOG" | grep "801DD680" | head -4
echo "===CMDTL368=== the ladder"
grep -n "cmdtl" "$LOG" | tail -8
echo "===PENDCLR368=== pendclr after the serve"
SL=$(grep -n "wserve" "$LOG" | head -1 | cut -d: -f1)
echo "SERVE_LINE=$SL"
grep -n "pendclr" "$LOG" | awk -v s="$SL" -F: '$1>=s' | head -6
echo "===FTAB368=== the file reads"
grep -n "READ ISSUED" "$LOG" | tail -4
echo "===FE1C368=== the FE1C walk (late)"
grep -n "chg. FE1C" "$LOG" | tail -6
echo "===TRAIL368=== the park census"
grep -n "trail. R642" "$LOG" | head -2
grep -n "trail. R642" "$LOG" | tail -2
echo "===FAULT368=== fault census"
...
echo "===VIS368=== exits + gpu census"
grep -n "rungasp" "$LOG" | tail -3
grep -n "gpufin" "$LOG" | tail -2
echo "===C368DONE=== R1365 run complete - the scheduled-read verdict comes from these receipts"
