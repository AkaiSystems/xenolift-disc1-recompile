#!/bin/bash
# c375_r1366.sh - THE R1366 PATCH+RUN CYCLE. The c373/c374
...
set -u
cd "$HOME/Downloads/xenolift" || { echo "C375-FAILED: xenolift dir missing"; exit 1; }
export LC_ALL=C
...
mkdir -p /tmp/c375_stub/mach-o
: > /tmp/c375_stub/mach-o/dyld.h
cc -fsyntax-only -std=gnu99 -I/tmp/c375_stub -Iruntime "$SRC" 2>/tmp/c375_parse_err.txt
PARSE_RC=$?
echo "PARSE_RC=$PARSE_RC"
if [ $PARSE_RC -ne 0 ]; then
  echo "PARSE-FAILED: restoring the pristine tree (fail-closed)"
  head -12 /tmp/c375_parse_err.txt
  cp -p "$PDIR/runtime.c.pre" "$SRC"
  echo "RESTORED"
  exit 0
fi
echo "PARSE-OK"
echo "===RUN375=== the R1366 build, 120s budget, first-fault stop"
RUN_BUDGET_S=120 ./run.sh > /tmp/run_full_c375.txt 2>&1
RUN_RC=$?
echo "RUN_RC=$RUN_RC"
...
if [ -s "$LOG" ]; then
  ...
  echo "===DEFIB375=== the carry receipts"
  grep -n "carry PROMOTE\|R473 stalled-load COMPLETED\|carry-v3" "$LOG" | head -12
  echo "===REHT375=== the alignment receipt (dest==header now)"
  grep -n "reshtail\]" "$LOG" | head -6
  echo "===LZHLE375=== the HLE decode + clean exit receipts"
  grep -n "lzss-hle\|dechist\|lzss-x\|R1320" "$LOG" | head -12
  echo "===UNPACK375=== the unpack receipts"
  grep -n "unpackw\|lzsscam\]" "$LOG" | tail -8
  echo "===FLDSTATE375=== the state-1 + install receipts"
  grep -n "statetbl\|pFnMain\|coordw\|fldsnap\|bankheal" "$LOG" | tail -10
  echo "===CMDTL375=== the command ladder"
  grep -n "cmdtl\]" "$LOG" | tail -10
  echo "===FTAB375=== the file reads"
  grep -n "READ ISSUED" "$LOG" | head -8
  echo "===FAULT375=== fault census"
  ...
  tail -20 /tmp/run_full_c375.txt
fi
echo "===POSTSHA375==="
shasum -a 256 "$SRC" | cut -d" " -f1
echo "===C375DONE=== R1366 cycle complete - the verdict comes from these receipts"
