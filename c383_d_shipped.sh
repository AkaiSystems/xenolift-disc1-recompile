#!/bin/bash
# c383_r1367.sh - THE R1367 PATCH+RUN CYCLE. The c381
...
set -u
cd "$HOME/Downloads/xenolift" || { echo "C383-FAILED: xenolift dir missing"; exit 1; }
export LC_ALL=C
...
PRE_G4=$(grep -c "108933-108994 band" "$SRC" || true)
echo "PRE R1367-cam-print count=$PRE_G4 (must be 0; informational: the generic 108995u-string pre-exists $(grep -c "108995u) {" "$SRC" || true) elsewhere - NEVER gate on it)"
...
while i < len(lines):
    ...
    if s.startswith("/* R554 FDF8-restore DE-NESTED"):
        cam = [
            IND + "/* R1367 (c383): THE PHANTOM-BACKLOG GUARD. The c376/c380/c381",
...
POST_G2=$(grep -c "108933-108994 band" "$SRC" || true)
POST_G2b=$(grep -c "belongs to a different request" "$SRC" || true)
POST_G3=$(grep -c "if (f_dst >= 0x801D9724u" "$SRC" || true)
echo "POST R1367 count=$POST_G1 (must be 3)"
echo "POST R1367-cam-print count=$POST_G2 (must be 1)"
echo "POST R1367-cam-text count=$POST_G2b (must be 1)"
echo "POST f_dst-if count=$POST_G3 (must be 1)"
if [ "$POST_G1" != "3" ] || [ "$POST_G2" != "1" ] || [ "$POST_G2b" != "1" ]; then
  echo "POST-GATE-FAILED: restoring the pristine tree"
  cp -p "$PDIR/runtime.c.pre" "$SRC"
  echo "RESTORED"
  exit 0
fi
cc -fsyntax-only -std=gnu99 -Wno-implicit-function-declaration -Wno-int-conversion -Wno-constant-conversion -Wno-int-to-void-pointer-cast "$SRC" 2>/tmp/c383_parse_err.txt
PARSE_RC=$?
echo "PARSE_RC=$PARSE_RC"
if [ $PARSE_RC -ne 0 ]; then
  echo "PARSE-FAILED: restoring the pristine tree (fail-closed)"
  head -12 /tmp/c383_parse_err.txt
  cp -p "$PDIR/runtime.c.pre" "$SRC"
  echo "RESTORED"
  exit 0
fi
echo "PARSE-OK"
echo "===RUN383=== the R1367 build, 120s budget, first-fault stop"
RUN_BUDGET_S=120 ./run.sh > /tmp/run_full_c383.txt 2>&1
RUN_RC=$?
echo "RUN_RC=$RUN_RC"
echo "===FRESHLOG383=== the tree-root log (the c376 lesson: never witness dirs)"
ls -la run.log run.log.d 2>/dev/null
LOG="run.log"
[ -s "$LOG" ] || LOG="run.log.d"
if [ ! -s "$LOG" ]; then
  echo "TREE-ROOT-LOG-MISSING: preserving run_full tail"
  tail -25 /tmp/run_full_c383.txt
  exit 0
fi
echo "AUTHORITATIVE LOG = $LOG ($(wc -l < "$LOG" | tr -d ' ') lines)"
echo "===R1367GUARD383=== the phantom-block receipts (expect ~2, one per boot)"
grep -n "R1367" "$LOG" | head -8
echo "===FE34FIRE383=== the fe34fix family (the phantom restores must be GONE)"
grep -n "fe34fix\]" "$LOG" | head -10
echo "===MEMBERWALK383=== the walk past member=6?"
grep -n "stab\] req member=" "$LOG" | tail -10
echo "===NEWSECT383=== the 239411 serve (the ReadN that never formed - did it?)"
grep -n "LBA 2394" "$LOG" | grep "loaded into data FIFO" | tail -10
echo "===FDF8ARM383=== the arm story tail"
grep -n "FDF8 ARM-CONTEXT" "$LOG" | tail -6
echo "===CMDTL383=== the ladder tail"
grep -n "cmdtl\]" "$LOG" | tail -10
echo "===NEWMOD383=== module announces"
grep -n "newmod\]" "$LOG" | tail -6
echo "===R1366FIRE383=== the whole-member carry (if the walk reaches file-14)"
grep -n "R1366" "$LOG" | head -6
echo "===LZHLE383=== the decode receipts"
grep -n "lzss-hle" "$LOG" | tail -8
echo "===WEDGE383=== the spin census (is the wedge dead?)"
echo "wedgespin-total $(grep -c "wedgespin" "$LOG" || true)"
grep -n "wedgespin" "$LOG" | tail -3
echo "===FAULTS383=== the fault census"
