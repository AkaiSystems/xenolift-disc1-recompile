#!/bin/bash
# c393b_r1369.sh - THE R1369 (reship: arm gate now a DELTA check - the real tree already holds another 0x8004FE1Cu) == 6u line, so the absolute POST gate refused; python now reads the pre-arm count from the live tree and requires pre+1) PATCH+RUN CYCLE. The c392
...
PRE_ARM=$(grep -c "0x8004FE1Cu) == 6u" "$SRC" || true)
echo "PRE arm-occurrences count=$PRE_ARM (post must be PRE+1)"
...
patched = "\n".join(out)
pre_arm = text.count("0x8004FE1Cu) == 6u")
if patched.count("R1369") != 3:
    print("POST-FAILED: R1369 count %d (must be 3)" % patched.count("R1369")); sys.exit(1)
if patched.count("0x8004FE1Cu) == 6u") != pre_arm + 1:
    print("POST-FAILED: arm delta != 1 (pre=%d post=%d)" % (pre_arm, patched.count("0x8004FE1Cu) == 6u"))); sys.exit(1)
print("PRE_ARM_DELTA_BASE=%d" % pre_arm)
...
POST_ARM_EXPECT=$((PRE_ARM+1))
POST_G3=$(grep -c "stale-empty entry" "$SRC" || true)
echo "POST R1369 count=$POST_G1 (must be 3)"
echo "POST arm count=$POST_G2 (must be $POST_ARM_EXPECT = PRE+1)"
echo "POST camera count=$POST_G3 (must be 1)"
if [ "$POST_G1" != "3" ] || [ "$POST_G2" != "$POST_ARM_EXPECT" ] || [ "$POST_G3" != "1" ]; then
