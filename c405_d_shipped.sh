#!/bin/bash
# c405_r1374.sh - THE R1374 PATCH+RUN CYCLE v2. c404 failed at
# its OWN post-gate (fail-closed, tree untouched): the 'old posture
# residue' check tested cd_last_cmd == 0x02u against the WHOLE tree,
# where the R955/R990 mvkick block legitimately contains it - the
# sandbox replica omitted that block so the dry-run could not catch
# it. v2 drops the redundant check (the R1373-residue==0 gate already
# proves the old block is gone) and dry-runs against a replica that
# INCLUDES the mvkick block. Same repair as c404 otherwise (one narrowly
