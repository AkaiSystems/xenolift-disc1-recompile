#!/bin/bash
# c439_r1379b.sh - HYBRID CENSUS + PATCH CYCLE (R1379
...
# stabilizes.
set -u
cd "$HOME/Downloads/xenolift" || { echo "C439-FAILED: xenolift dir missing"; exit 1; }
export LC_ALL=C
...
echo "===PREGATE439=== pre-existence gates (fail-closed)"
...
echo "===CENSUS439=== the two discard conditions (printed by the patch logic)"
echo "===APPLY439=== strict shape gate (true anchor) + conditional apply"
...
echo "===POSTGATE439=== post-apply marker gates"
...
echo "===BUILD439=== compile the patched runtime"
if ! ./run.sh build > /tmp/c439_build.txt 2>&1; then
  echo "BUILD-FAILED: restoring baseline"; tail -25 /tmp/c439_build.txt; cp -p runtime/runtime.c.pre_r1379 "$SRC"; exit 0
fi
echo "BUILD OK"
tail -3 /tmp/c439_build.txt
echo "===RUN439=== full run (RUN_BUDGET_S=90)"
RUN_BUDGET_S=90 ./run.sh run > /tmp/run_full_c439.txt 2>&1
echo "RUN_RC=$?"
LOG="run.log"
echo "===DIGEST439==="
...
echo "===PRESERVE439=== c118 protocol"
D=$(mktemp -d "$PWD/c439_snap.XXXXXX") || { echo "SNAPSHOT-DIR-FAILED"; exit 0; }
cp -p run.log "$D/run.log" && CP1=0 || CP1=1
cp -p /tmp/run_full_c439.txt "$D/run_full_c439.txt" && CP2=0 || CP2=1
echo "cp_rcs: $CP1 $CP2"
ls -la "$D"
shasum -a 256 "$D/run.log" "$D/run_full_c439.txt"
tail -8 "$D/run_full_c439.txt"
echo "===C439DONE=== R1379b shipped - PASS = no discard at live posture + walk past LBA 9 + mount stabilizes; REVERT = discard suppressed but walk still stalls (next: re-serve the LBA-9 sector)"
