...
# /tmp/c456_receipts.txt AS IT RUNS so a watchdog kill
...
export LC_ALL=C
exec > >(tee /tmp/c456_receipts.txt) 2>&1
...
echo "===RUN456=== run-only (RUN_BUDGET_S=190, no build)"
RUN_BUDGET_S=190 ./run.sh run > /tmp/run_full_c456.txt 2>&1
echo "RUN_RC=$?"
LOG="run.log"
echo "===DIGEST456==="
...
echo "===PRESERVE456=== c118 protocol"
D=$(mktemp -d "$PWD/c456_snap.XXXXXX") || { echo "SNAPSHOT-DIR-FAILED"; exit 0; }
cp -p run.log "$D/run.log" && CP1=0 || CP1=1
cp -p /tmp/run_full_c456.txt "$D/run_full_c456.txt" && CP2=0 || CP2=1
cp -p /tmp/c456_receipts.txt "$D/c456_receipts.txt" && CP3=0 || CP3=1
echo "cp_rcs: $CP1 $CP2 $CP3"
ls -la "$D"
shasum -a 256 "$D/run.log" "$D/run_full_c456.txt"
tail -8 "$D/run_full_c456.txt"
echo "===C456DONE=== the 150s run receipted - PASS/REVERT from the digest receipts"
