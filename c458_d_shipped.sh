...
# tree (ef5508ce verified first). NO patch, NO build:
...
# /tmp/c458_receipts.txt AS IT RUNS so a watchdog kill
...
export LC_ALL=C
exec > >(tee /tmp/c458_receipts.txt) 2>&1
SRC="runtime/runtime.c"
EXPECT="ef5508cecdb61e35bf5307cf7b887fa59b776932a71df5896d2d7189bd631e13"
SS=$(shasum -a 256 "$SRC" | cut -d" " -f1)
echo "SRC_SHA=$SS"
if [ "$SS" != "$EXPECT" ]; then echo "GATE-FAILED: runtime.c is not the R1383 tree ef5508ce - refusing to run"; exit 0; fi
echo "TREE_VERIFIED (the R1383 tree - the arm+bell is in place)"
C0=$(grep -c -e "f15arm" "$SRC")
echo "f15arm marker: $C0 (expect 1)"
if [ "$C0" != "1" ]; then echo "GATE-FAILED: marker missing"; exit 0; fi
C1=$(grep -c -e "R1383 (c456 receipts)" "$SRC")
echo "R1383 marker: $C1 (expect 1)"
if [ "$C1" != "1" ]; then echo "GATE-FAILED: R1383 marker missing"; exit 0; fi
C2=$(grep -c -e "r1381_since >= 2" "$SRC")
echo "freeze-2s line: $C2 (expect 1)"
if [ "$C2" != "1" ]; then echo "GATE-FAILED: freeze line not the 2s form"; exit 0; fi
...
echo "===RUN458=== run-only (RUN_BUDGET_S=190, no build)"
RUN_BUDGET_S=190 ./run.sh run > /tmp/run_full_c458.txt 2>&1
echo "RUN_RC=$?"
LOG="run.log"
echo "===DIGEST458==="
...
echo "--- tree sha:"
echo "expected=ef5508cecdb61e35bf5307cf7b887fa59b776932a71df5896d2d7189bd631e13"
shasum -a 256 "$SRC" | cut -d" " -f1
echo "===PRESERVE458=== c118 protocol"
D=$(mktemp -d "$PWD/c458_snap.XXXXXX") || { echo "SNAPSHOT-DIR-FAILED"; exit 0; }
cp -p run.log "$D/run.log" && CP1=0 || CP1=1
cp -p /tmp/run_full_c458.txt "$D/run_full_c458.txt" && CP2=0 || CP2=1
cp -p /tmp/c458_receipts.txt "$D/c458_receipts.txt" && CP3=0 || CP3=1
echo "cp_rcs: $CP1 $CP2 $CP3"
ls -la "$D"
shasum -a 256 "$D/run.log" "$D/run_full_c458.txt"
tail -8 "$D/run_full_c458.txt"
echo "===C458DONE=== the 150s run receipted - PASS/REVERT from the digest receipts"
