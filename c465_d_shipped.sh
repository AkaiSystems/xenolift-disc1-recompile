...
# tree (92094801 verified first). NO patch, NO build:
...
# /tmp/c465_receipts.txt AS IT RUNS so a watchdog kill
...
export LC_ALL=C
exec > >(tee /tmp/c465_receipts.txt) 2>&1
SRC="runtime/runtime.c"
EXPECT="92094801fa5c24d4d205da04b5ae313269bcdc55e5af9883f71672ce8ebdddad"
SS=$(shasum -a 256 "$SRC" | cut -d" " -f1)
echo "SRC_SHA=$SS"
if [ "$SS" != "$EXPECT" ]; then echo "GATE-FAILED: runtime.c is not the R1385 tree 92094801 - refusing to run"; exit 0; fi
echo "TREE_VERIFIED (the R1385 tree - the arm+bell+announcement is in place)"
C0=$(grep -c -e "f15arm" "$SRC")
echo "f15arm marker: $C0 (expect 1)"
if [ "$C0" != "1" ]; then echo "GATE-FAILED: marker missing"; exit 0; fi
C1=$(grep -c -e "R1385 (c463 receipts)" "$SRC")
echo "R1385 marker: $C1 (expect 1)"
if [ "$C1" != "1" ]; then echo "GATE-FAILED: R1385 marker missing"; exit 0; fi
C2=$(grep -c -e "f15post. R1385" "$SRC")
echo "f15post print: $C2 (expect 1)"
if [ "$C2" != "1" ]; then echo "GATE-FAILED: f15post print missing"; exit 0; fi
C3=$(grep -c -e "r1385_announced" "$SRC")
echo "r1385_announced refs: $C3 (expect 4)"
if [ "$C3" != "4" ]; then echo "GATE-FAILED: r1385 refs wrong"; exit 0; fi
...
echo "===RUN465=== run-only (RUN_BUDGET_S=190, no build)"
RUN_BUDGET_S=190 ./run.sh run > /tmp/run_full_c465.txt 2>&1
echo "RUN_RC=$?"
LOG="run.log"
echo "===DIGEST465=== the R1385 verdict: the completion announcement + the wait's own camera"
...
echo "--- tree sha:"
echo "expected=92094801fa5c24d4d205da04b5ae313269bcdc55e5af9883f71672ce8ebdddad"
shasum -a 256 "$SRC" | cut -d" " -f1
echo "===PRESERVE465=== c118 protocol"
D=$(mktemp -d "$PWD/c465_snap.XXXXXX") || { echo "SNAPSHOT-DIR-FAILED"; exit 0; }
cp -p run.log "$D/run.log" && CP1=0 || CP1=1
cp -p /tmp/run_full_c465.txt "$D/run_full_c465.txt" && CP2=0 || CP2=1
cp -p /tmp/c465_receipts.txt "$D/c465_receipts.txt" && CP3=0 || CP3=1
echo "cp_rcs: $CP1 $CP2 $CP3"
ls -la "$D"
shasum -a 256 "$D/run.log" "$D/run_full_c465.txt"
tail -8 "$D/run_full_c465.txt"
echo "===C465DONE=== the 150s run receipted - PASS/REVERT from the digest receipts"
