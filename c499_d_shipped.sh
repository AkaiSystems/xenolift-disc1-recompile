...
# /tmp/c499_receipts.txt. Pure-ASCII output.
set -u
cd "$HOME/Downloads/xenolift" || { echo "C499-FAILED: xenolift dir missing"; exit 1; }
export LC_ALL=C
exec > >(tee /tmp/c499_receipts.txt) 2>&1
...
echo "===LOCATE499=== xenolift_dispatch definition + param name (fail-closed)"
DEF_LN=""
DEF_BRACE=""
for L in $(grep -n -e "void xenolift_dispatch(uint32_t" "$SRC" | cut -d: -f1 | head -8); do
  THIS=$(sed -n "${L}p" "$SRC")
...
if [ -z "$DEF_LN" ]; then echo "VALID-FAILED: xenolift_dispatch definition not found - refusing"; grep -n -e "xenolift_dispatch(uint32_t" "$SRC" | head -6; exit 0; fi
DEF_LINE=$(sed -n "${DEF_LN}p" "$SRC")
PN=$(printf "%s" "$DEF_LINE" | sed "s/.*uint32_t *\([a-zA-Z_][a-zA-Z0-9_]*\).*/\1/")
if [ -z "$PN" ]; then echo "VALID-FAILED: could not extract the dispatch param name from: $DEF_LINE"; exit 0; fi
echo "dispatch definition at line $DEF_LN (brace on $DEF_BRACE line), param=[$PN]"
echo "===GTEEX499=== the runtime's own GTE compute-op call (from the COP2 region)"
...
echo "===PATCH499=== build the R1394 head + tail + the dispatch splice"
...
echo "===SYNCHK499=== HARD GATE syntax check (unpiped rc)"
...
echo "SYNCHK OK"
echo "===RUN499=== build + the 190s verdict run"
...
echo "===C499DONE=== PASS = [ovlint] entries + either forward execution or an honest named-stop receipt - digest is pure ASCII"
