#!/bin/bash
# c499_patch.sh - STAGE B-2 v1: THE MODULE-WINDOW
# INTERPRETER (R1394). Jos's mandate: a correct
# interpreter fallback instead of executing an unrelated
# translation - and instead of stopping. The c497 guard
# STOPS wrong-module dispatches; R1394 EXECUTES the
# installed module's live bytes. Design from the c498
# census receipts ONLY: guest regs = the global r[32];
# memory via xenolift_mem_* (every HLE hook stays live);
# the guard sits at the HEAD of xenolift_dispatch
# (xenolift_trace is inside the emitted fn - an
# interpreter returning there would run the wrong body
# after). CONTROL: a jump/jr OUT of the window = the
# module hands control away - tail-dispatch natively; a
# jal OUT = a CALL into kernel code - dispatch natively
# (the kick pattern) and RESUME at the link address. v1
# LIMITS (receipted): local hi/lo; COP0 unsupported; a
# 4M instruction budget per call; unhandled opcode -> the
# explicit [ovlstop] (capture + exit 99), never silent
# execution. The GTE compute-op call is EXTRACTED from
# the runtime's own COP2 region (23434-23680); fail-closed
# to an explicit unsupported-stop if not exactly one
# match. Hard-gated unpiped syntax check; build + the 190s
# verdict run + digest. Fail-closed, tee'd to
# /tmp/c500_receipts.txt. Pure-ASCII output.
set -u
cd "$HOME/Downloads/xenolift" || { echo "C499-FAILED: xenolift dir missing"; exit 1; }
export LC_ALL=C
exec > >(tee /tmp/c500_receipts.txt) 2>&1
...
echo "===LOCATE500=== xenolift_dispatch definition + param name (adaptive, fail-closed)"
echo "--- every xenolift_dispatch line in runtime/runtime.c (candidates named unconditionally):"
grep -n -e "xenolift_dispatch" "$SRC" | grep -v -e "g_guest_depth" | head -20
DEF_LN=""
DEF_BRACE=""
DEF_FILE="$SRC"
for L in $(grep -n -e "xenolift_dispatch" "$SRC" | cut -d: -f1 | head -60); do
  THIS=$(sed -n "${L}p" "$SRC")
  case "$THIS" in
    *"xenolift_dispatch(0x"*|*"g_guest_depth"*|*");"*) continue ;;
  esac
  case "$THIS" in
    *"xenolift_dispatch"*"("*) ;;
    *) continue ;;
  ...
  NEXT=$(sed -n "$((L+1))p" "$SRC")
  case "$NEXT" in *"{"*) if [ -z "$DEF_LN" ]; then DEF_LN=$L; DEF_BRACE=next; fi ;; esac
done
if [ -z "$DEF_LN" ]; then
  echo "--- runtime.c has no brace-bearing candidate; searching the other runtime sources:"
  for F in runtime/*.c; do
    if [ "$F" != "$SRC" ]; then grep -n -e "xenolift_dispatch" "$F" | head -8; fi
  done
  echo "VALID-FAILED: the dispatch definition was not found - nothing patched"
  exit 0
fi
DEF_LINE=$(sed -n "${DEF_LN}p" "$SRC")
...
echo "dispatch definition at line $DEF_LN (brace on $DEF_BRACE line), param=[$PN], line: $DEF_LINE"
echo "===GTEEX500=== the runtime's own GTE compute-op call (from the COP2 region)"
...
if [ -n "$GTE_CALL" ]; then
  echo "(wired: the interpreter calls the runtime's own GTE exec)"
else
  echo "(absent/ambiguous: the interpreter ships with the explicit unsupported-GTE stop; the census print below names the wiring for v2)"
fi
sed -n "${COP2_LN},$((COP2_LN+260))p" "$SRC" | grep -n -e "hle_gte" | head -14
echo "===PATCH500=== build the R1394 head + tail + the dispatch splice"
...
static int r1394_dispatch_guard(unsigned int t)
{
    unsigned char lw[16];
    int rc;
...
/* R1394 (c499): the module-window interpreter, stage B-2 v1. The dispatch
 * guard (see the definition at the end of this file) sits at the HEAD of
 * xenolift_dispatch: it compares the live module window against the emit-era reference
 * (emit_ref.bin, the pSYW6z window receipted at the fault addresses) and
 * on mismatch INTERPRETS the installed module's live bytes instead of
 * running the unrelated emitted translation. Full design notes + the v1
 * limits (local hi/lo, COP0 unsupported, 4M-instruction budget, explicit
 * [ovlstop] fail-safe) are in the definition at the end of this file. */
static int r1394_dispatch_guard(unsigned int t);
BLOCKEOF
...
INS=$(( INS + HLINES ))
awk -v L="$INS" -v PN="$PN" 'NR==L {print; print "    if (r1394_dispatch_guard(" PN ")) { return; }"; next} {print}' /tmp/rt_p1.c > /tmp/rt_p2.c
{ cat /tmp/rt_p2.c; cat /tmp/r1394_tail.c; } > /tmp/rt_new.c
mv /tmp/rt_new.c "$SRC"
echo "===GATES500==="
E_GRD=$(grep -c -e "r1394_dispatch_guard" /tmp/r1394_tail.c | tr -d " ")
A_GRD_TOT=$(grep -c -e "r1394_dispatch_guard" "$SRC" | tr -d " ")
E_GRD=$((E_GRD + 1 + 1))
...
echo "guard refs: expect $E_GRD got $A_GRD_TOT (def+proto+splice)"
echo "interp refs: expect $E_INT got $A_INT_TOT (def+proto+guard-call)"
echo "ovlint receipt sites: $A_OVLINT"
echo "the splice line: $(sed -n "${INS}p" "$SRC")"
if [ "$A_GRD_TOT" != "$E_GRD" ] || [ "$A_INT_TOT" != "$E_INT" ]; then
  echo "VALID-FAILED: gate mismatch - RESTORING the pre-c499 tree"
  cp -p "$SRC.pre_c499" "$SRC"
  exit 0
fi
echo "GATES OK - the tree carries R1394"
NEW=$(shasum -a 256 "$SRC" | cut -d" " -f1)
echo "NEW_TREE_SHA=$NEW"
echo "===SYNCHK500=== HARD GATE syntax check (unpiped rc)"
cc -fsyntax-only "$SRC" > /tmp/c499_syn.txt 2>&1
SYN_RC=$?
head -10 /tmp/c499_syn.txt
echo "SYNCHK_RC=$SYN_RC"
if [ "$SYN_RC" != "0" ]; then
  echo "VALID-FAILED: the patched tree does not parse - RESTORING the pre-c499 tree"
  cp -p "$SRC.pre_c499" "$SRC"
  exit 0
fi
echo "SYNCHK OK"
echo "===RUN500=== build + the 190s verdict run"
...
echo "===C500DONE=== PASS = [ovlint] entries + either forward execution or an honest named-stop receipt - digest is pure ASCII"
