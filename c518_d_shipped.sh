#!/bin/bash
# c518_patch.sh - THE POST-CONVERSION COLLECTOR PASS (R1380),
...
# Fail-closed, tee'd to /tmp/c518_receipts.txt. Pure-ASCII
# output.
set -u
cd "$HOME/Downloads/xenolift" || { echo "C518-FAILED: xenolift dir missing"; exit 1; }
export LC_ALL=C
exec > >(tee /tmp/c518_receipts.txt) 2>&1
...
if grep -q "R1380" "$SRC"; then echo "VALID-FAILED: R1380 already present - refusing (idempotent)"; exit 0; fi
...
PYEOF
if [ "$?" != "0" ]; then echo "VALID-FAILED: splice failed - RESTORING"; cp -p "$SRC.pre_c518" "$SRC"; exit 0; fi
echo "===GATES518==="
E_R1380=$(grep -c -e "R1380" /tmp/r1380_block.c | tr -d " ")
A_R1380=$(grep -c -e "R1380" "$SRC" | tr -d " ")
...
echo "R1380 refs: expect $E_R1380 got $A_R1380"
echo "postconv refs: expect $E_POST got $A_POST"
...
if [ "$A_R1380" != "$E_R1380" ] || [ "$A_POST" != "$E_POST" ] || [ "$A_L1" != "1" ] || [ "$A_L2" != "1" ] || [ "$A_L3" != "1" ]; then
  echo "VALID-FAILED: gate mismatch - RESTORING the pre-c518 tree"
  cp -p "$SRC.pre_c518" "$SRC"
  exit 0
fi
echo "GATES OK"
...
echo "===SYNCHK518=== HARD GATE syntax check (unpiped rc)"
...
echo "===RUN518=== build + the 190s verdict run"
...
echo "===C518DONE=== PASS = [postconv] slots=4 receipt + h4 dispatch + cd-dma at LBA 109158 + FDF8 countdown + the file#18 member consumed + forward execution past the unpack-on-zeros - digest is pure ASCII"
