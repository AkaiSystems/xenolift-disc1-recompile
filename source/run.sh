...
RUN_BUDGET_S=${RUN_BUDGET_S:-470} # R1448 (c1048): the budget raise - the 278 default+22 fuse = the 302s wall that starved the movie era mid-frame (FDF8=9048 armed at kill); the R1417 window already targets the 540s daemon (WIN_LEFT=500-compile-20~474), so 470 fits the window with the 20s digest margin; fresh-emit cycles keep the R784 120s cap
...
WIN_LEFT=$(( 500 - SECONDS - 20 ))  # R1417 (c913) v11.6 window alignment: the daemon execute window is now 540s (R1416); target 500 keeps the 20s digest margin + compile headroom under it - the old 200s target capped every run to 177+22=199s since c372
