...
BIN="$HOME/Desktop/PS7Z/PS1Games/Xenogears (USA) (Disc 1)/Xenogears (USA) (Disc 1)/Xenogears (USA) (Disc 1).bin"
...
# R750/R752 GATED BYPASS, DEFINITION-ANCHORED + NO-CHURN. c325: name-anchored
  ...
  if [ "$NZ" -gt 4000 ]; then
    if [ ! -f stage2_region.bin ]; then
      ...
      if [ "$DRIFT" -gt 20 ] || [ "$DRIFT" -lt -20 ]; then
        cp stage2_field.bin stage2_region.bin; echo "stage2: field-era capture promoted to stage2_region.bin ($NZ nonzero hex chars, drift ${DRIFT}% > 20% era-jump gate)"
      else
